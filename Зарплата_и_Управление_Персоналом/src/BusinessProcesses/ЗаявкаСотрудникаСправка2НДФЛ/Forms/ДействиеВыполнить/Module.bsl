...
Процедура ПодписатьЭП(Команда)
	...
	ДополнительныеПараметры.Вставить("МассивФайловСправок", МассивФайловСправок);
	...
	Попытка
		Для Каждого СтрокаТЧ Из Задание.СправкиНДФЛ Цикл	
			...
			Если ФайлыСправок.Количество() > 0 Тогда
				Для Каждого ФайлСправки Из ФайлыСправок Цикл
					ФайлСправкиОбъект = ФайлСправки.ПолучитьОбъект();
					ФайлСправкиОбъект.ПометкаУдаления = Истина;
...
Процедура ЗаполнитьЭлементыФайлаОтвета(ФайлОтветаЭлемент, ФайлОтветаСсылка, ИмяРеквизита, ЗначенияРеквизитов)
	...
	ФайлСЭП = (Задание.ВариантФормированияФайлаОтвета =
				ПредопределенноеЗначение("Перечисление.ВариантыФормированияФайлаОтветаЗаявкиСотрудника.ФайлСЭП"));
	ФайлСФаксимиле = (Задание.ВариантФормированияФайлаОтвета =
						ПредопределенноеЗначение("Перечисление.ВариантыФормированияФайлаОтветаЗаявкиСотрудника.СканСПодписьюИПечатью"));				
				
	Элементы["ВидДокумента" + ИмяРеквизита].Видимость = Не ФайлПустой;
	
	ЭлементВыбрать = Элементы["Выбрать" + ИмяРеквизита]; 
	ЭлементВыбрать.Видимость = ФайлСФаксимиле И Не Объект.Выполнена;
	ЭлементВыбрать.Доступность = Не ФайлСодержитЭП;
...
&НаСервере
Функция СостояниеФайловОтвета(ТребуетсяЭП)
		
	СостояниеФайлов = Новый Структура;
	СостояниеФайлов.Вставить("Сформированы", Истина);
	Если ТребуетсяЭП Тогда
		СостояниеФайлов.Вставить("Подписаны", Истина);
	КонецЕсли;
	СостояниеФайлов.Вставить("ФайлыОтвета", Новый Массив);
	
	МассивСправок = Задание.СправкиНДФЛ.Выгрузить().ВыгрузитьКолонку("СправкаНДФЛ");
	РезультатЗапроса = РезультатЗапросаФайловОтвета(МассивСправок);
	Если РезультатЗапроса.Пустой() Тогда
		СостояниеФайлов.Сформированы = Ложь;
		Если ТребуетсяЭП Тогда
			СостояниеФайлов.Подписаны = Ложь;
		КонецЕсли;
		Возврат СостояниеФайлов;
	КонецЕсли;
	
	МассивФайловОтвета = Новый Массив;
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		...
		МассивФайловОтвета.Добавить(Выборка.ПрисоединенныйФайл);
	...
	СостояниеФайлов.ФайлыОтвета = МассивФайловОтвета;
	Возврат СостояниеФайлов;
	
КонецФункции

&НаСервере
Функция РезультатЗапросаФайловОтвета(СправкиНДФЛ)
...
Процедура ИнициализироватьФормуЗадачи()
	...
	ФайлСФаксимиле = (Задание.ВариантФормированияФайлаОтвета =
						ПредопределенноеЗначение("Перечисление.ВариантыФормированияФайлаОтветаЗаявкиСотрудника.СканСПодписьюИПечатью"));
	Элементы.Памятка.Видимость = ИспользуетсяКадровыйЭДО И Не ФайлСФаксимиле;
...
Процедура УстановитьВидимостьКнопокДействий()
	...
	БизнесПроцессыЗаявокСотрудниковФормы.УстановитьВидимостьКнопокДействий(ЭтотОбъект, Задание.СправкиНДФЛ);
	
	Если Объект.Выполнена Тогда
		Элементы.СформироватьФайлы.Видимость = Ложь;
	...
	СостояниеФайловОтвета = СостояниеФайловОтвета(ТребуетсяЭП);
	
	Элементы.Выполнено.Доступность = Элементы.Выполнено.Доступность И (Задание.СправкиНДФЛ.Количество() > 0);
	Если ТребуетсяЭП Тогда
		Элементы.ПодписатьЭП.Доступность = Не СостояниеФайловОтвета.Подписаны И СостояниеФайловОтвета.Сформированы;
		Элементы.Выполнено.Доступность = Элементы.Выполнено.Доступность И СостояниеФайловОтвета.Подписаны;
	Иначе
		Элементы.Выполнено.Доступность = Элементы.Выполнено.Доступность И СостояниеФайловОтвета.Сформированы;
	КонецЕсли;
	Элементы.Выполнено.Доступность = Элементы.Выполнено.Доступность Или Не ИспользоватьНачислениеЗарплаты;
	
	Элементы.СоздатьДокумент.Доступность = Элементы.СоздатьДокумент.Доступность И ИспользоватьНачислениеЗарплаты;
									 
	Элементы.СформироватьФайлы.Доступность = Не СостояниеФайловОтвета.Сформированы;
	Элементы.СформироватьФайлы.Видимость = (Задание.СправкиНДФЛ.Количество() > 0) И Не ИспользуетсяКадровыйЭДО;
	
	ФайлСФаксимиле = (Задание.ВариантФормированияФайлаОтвета =
						ПредопределенноеЗначение("Перечисление.ВариантыФормированияФайлаОтветаЗаявкиСотрудника.СканСПодписьюИПечатью"));
	Элементы.ПодписатьЭП.Видимость = (ФайлСЭП И Не ИспользуетсяКадровыйЭДО) Или (ФайлСФаксимиле И ИспользуетсяКадровыйЭДО); 
	Если ФайлСФаксимиле Тогда
		ФайлыПриложеныПользователем = (СостояниеФайловОтвета.ФайлыОтвета.Количество() > 0);
		Для Каждого ФайлОтветаПоЗаявке Из СостояниеФайловОтвета.ФайлыОтвета Цикл
			ФайлыПриложеныПользователем = ФайлыПриложеныПользователем И Не КадровыйЭДО.ЭтоФайлПечатнойФормы(ФайлОтветаПоЗаявке);		
		КонецЦикла;
		Элементы.ПодписатьЭП.Доступность = ФайлыПриложеныПользователем; 
	КонецЕсли;
