...
Функция ДоступноВыполнение() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО (СправкаНДФЛПрисоединенныеФайлы.ВладелецФайла = СправкаНДФЛ.Ссылка)
	               |ГДЕ
	...
	ФайлыОтвета = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ПрисоединенныйФайл");
	
	Возврат БизнесПроцессыЗаявокСотрудников.ДоступноВыполнение(ЭтотОбъект, 
															   ТаблицаЗначенийСправкиНДФЛ, 
															   ФайлыОтвета);	
КонецФункции
