...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИсправитьСтатусыОтчетаРосстат();
	
КонецПроцедуры

&НаСервере
Процедура ИсправитьСтатусыОтчетаРосстат()
	...
	Попытка
		...
		Запрос.Текст = 
			...
			|	И ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПротоколВходногоКонтроляОтчетностиФСГС)
			|	И ЖурналОтчетовСтатусы.СостояниеСдачиОтчетности = ЗНАЧЕНИЕ(Перечисление.СостояниеСдачиОтчетности.ДокументооборотНачат)
			|	И МЕСЯЦ(ТранспортноеСообщение.ДатаТранспорта) = 2
			|	И ГОД(ТранспортноеСообщение.ДатаТранспорта) = 2022";
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Сейчас = РезультатЗапроса.Выбрать();
		
		Пока Сейчас.Следующий() Цикл
			
			Орган = XMLСтрока(Сейчас.ВидКонтролирующегоОргана);
			ДолжноБыть = ДлительнаяОтправкаВызовСервера.ТекущийЭтапОтправки(Сейчас.Отчет, Орган);
			
			Если ДолжноБыть = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Сдан   = Перечисления.СостояниеСдачиОтчетности.ПоложительныйРезультатДокументооборота;
			НеСдан = Перечисления.СостояниеСдачиОтчетности.ОтрицательныйРезультатДокументооборота;
			
			СостояниеДолжноБыть = ДолжноБыть.СостояниеСдачиОтчетности;
			
			СостояниеДолжноБытьФинальным = 
				СостояниеДолжноБыть = Сдан
				ИЛИ СостояниеДолжноБыть = НеСдан;
			
			СкорректироватьСтатус = СостояниеДолжноБытьФинальным;
			
			Если СкорректироватьСтатус Тогда
				
				Отказ = Ложь;
				РегламентированнаяОтчетность.ЗаписьОбъектовРегламентированнойОтчетности(Сейчас.Отчет, Отказ);
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если ТекущиеДанныеНаименованиеОтчета = "Реестр для налоговых вычетов по акцизам на виноград" Тогда
			...
			Если ТекущиеДанныеНаименованиеОтчета
				= "Реестр для налоговых вычетов по акцизам на виноматериалы, виноградное и фруктовое сусло" Тогда
				ПоказатьПредупреждение(, НСтр(
				"ru='Проверка выгрузки в электронном виде формы
				|отчета, утвержденной приказом ФНС России
				|от 26.04.2021 № ЕД-7-15/419@, будет
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Информация о параметрах доступа к информационной системе организации"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.инф.о пар.дост.к ИС") Тогда
...
Процедура ВыгрузкаРегламентированногоОтчета(Ссылка, ОрганизацияСсылка, НаименованиеОтчета, ВариантОтчета, ПредставлениеПериода);
	...
	Если НаименованиеОтчета = "Реестр для налоговых вычетов по акцизам на виноград" Тогда
		ПоказатьПредупреждение(, НСтр(
