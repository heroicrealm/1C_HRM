...
Функция ВыгрузитьЭлементВФайлОбмена(
	...
	Если НЕ ЗначениеЗаполнено(КодНалоговогоОрганаПолучателя) Тогда
		Если ОпределятьКодНалоговогоОрганаПолучателя И ЗначениеЗаполнено(ДанныеСправочника.Организация) Тогда
			...
			СвойстваОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеСправочника.Организация,
				"КрупнейшийНалогоплательщик, КодНалоговогоОрганаПолучателя");
			УстановитьПривилегированныйРежим(Ложь);
			КодНалоговогоОрганаПолучателя = ?(СвойстваОрганизации.КрупнейшийНалогоплательщик,
				СокрЛП(СвойстваОрганизации.КодНалоговогоОрганаПолучателя), ДанныеСправочника.КодНО);
		Иначе
...
Функция ПроверитьВозможностьВыгрузки(ОбъектИлиСсылка) Экспорт
	...
	Если ТребоватьОГРНДляПредставителяФЛ Тогда
		Если НЕ Представитель_ЮридическоеЛицо И НЕ ЗначениеЗаполнено(Доверенность.ПредставительФЛ_ОГРН) Тогда
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
