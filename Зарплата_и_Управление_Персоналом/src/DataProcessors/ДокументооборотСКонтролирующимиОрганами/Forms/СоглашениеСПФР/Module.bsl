...
Функция СформироватьЗаявлениеПФР()
	...
	Если ЗначениеЗаполнено(Заявление) Тогда
		МестоХраненияКлюча = КриптографияЭДКОКлиентСервер.ЗаполнитьМестоХраненияКлюча(Заявление.МодельХраненияЗакрытогоКлюча, 
	...
	Если Заявление <> Неопределено И НЕ КриптографияЭДКОКлиентСервер.ЭтоЛокальнаяПодпись(МестоХраненияКлюча)
