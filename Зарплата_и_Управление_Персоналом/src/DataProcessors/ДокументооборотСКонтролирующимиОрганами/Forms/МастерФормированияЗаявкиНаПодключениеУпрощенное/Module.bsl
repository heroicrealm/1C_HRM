...
Процедура ИсправитьОшибкиЛокальногоХраненияКлюча(ДополнительныеПараметры, СтандартнаяОбработка = Истина)
	...
	ИначеЕсли Действие = "Установить криптопровайдер" Тогда
		...
		Если УЦ = ПредопределенноеЗначение("Перечисление.УдостоверяющиеЦентрыБРО.УЦФНС") Тогда
			ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.Мастер_УстановитеКриптоПро",,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
...
Процедура ПриУстановкеРежимаТолькоСУЦАсинхронно(
	...
	Если СертификатДолженБытьПолученОтГосУЦ ИЛИ ЭтоИнтеграцияСБанком Тогда
		ПереключитьНаЛокальноеХранение();
	Иначе
...
Процедура СохранитьЗаявление(ЗаписатьЗаявление = Истина, НовыйGUID = Ложь) Экспорт
	...
	Иначе
		...
		СоздатьНовыйДокументЗаявление_Контакты(НовыйДокументЗаявление);
...
Процедура СохранитьЗаявление_ДанныеОрганизации(НовыйДокументЗаявление)
	...
	СоздатьНовыйДокументЗаявление_Адрес(НовыйДокументЗаявление);
	
КонецПроцедуры

&НаСервере
Процедура СоздатьНовыйДокументЗаявление_Контакты(НовыйДокументЗаявление)
...
&НаСервере
Процедура СоздатьНовыйДокументЗаявление_Адрес(НовыйДокументЗаявление)
...
Процедура ОтработатьИзменениеОрганизации(ИННОрганизации = "")
	...
	ОпределятьВозможностьБезбумажки = Истина;
	ПриУстановкеРежимаТолькоСУЦАсинхронно(, ОпределятьВозможностьБезбумажки); // Асинхронно
...
Процедура ИзменитьОформлениеВладельцаЭП()
	...
	ПоказатьКакПоляФИО = 
		НЕ ПоказатьКакНадпись 
		И НЕ ЗначениеЗаполнено(ВладелецЭЦП) 
		
		И НЕ ЭтоДругойСотрудник(ЭтотОбъект);
	...
	Элементы.ГруппаВыбораВидаВладельцаЭЦП.Видимость = ЭтоЮридическоеЛицо И НЕ ЭтоИнтеграцияСБанком;
...
Процедура ИзменитьОформлениеТелефонаИПочтыДляПаролей(Форма)
	...
	ИздатьНовый = Форма.СпособПолученияСертификата = ПредопределенноеЗначение("Перечисление.СпособПолученияСертификата.ИздатьНовый");
	НуженТелефонДляСвязи = 
		Форма.ЭтоСертификатДругогоУЦ 
		ИЛИ Форма.СертификатДолженБытьПолученОтГосУЦ
		ИЛИ Форма.ЭтоИнтеграцияСБанком;
	
	Элементы.ГруппаПроверкаТелефона.Видимость = ИздатьНовый ИЛИ НуженТелефонДляСвязи;
...
Процедура ПроверитьВозможностьИспользованияЛокальногоКлюча(ПриОткрытии)
	...
	Если (ЗаявлениеСозданоКопированием ИЛИ ЭтоОткрытиеЗаявления)
		И УЦ = Перечисления.УдостоверяющиеЦентрыБРО.УЦФНС Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуКонтактнойИнформацииЗавершение_ПослеИзмененияЮрАдреса(Результат, ВходящийКонтекст) Экспорт
	...
	// Определяем, т.к. в регионе могли выключить возможность бесшовного получения сертификата
	ОпределятьВозможностьБезбумажки = Истина;
	ПриУстановкеРежимаТолькоСУЦАсинхронно(, ОпределятьВозможностьБезбумажки);
