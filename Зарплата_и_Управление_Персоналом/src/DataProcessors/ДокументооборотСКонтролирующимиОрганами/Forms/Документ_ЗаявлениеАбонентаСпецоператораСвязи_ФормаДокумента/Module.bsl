...
Процедура СформироватьТаблицуИзмененныхДанных()
	...
	Если (ПереходВОблако ИЛИ КриптографияЭДКОКлиентСервер.ЭтоПодписьСервиса(МестоХраненияКлюча)) 
		...
		КонецЕсли;
		ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.ПодтверждениеОперацийСКлючом, ЗаголовокПодтверждения);
	КонецЕсли;
	...
	Если ЭтоАдресПоФИАСу Тогда
		...
		ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.Область,Область,,,,,ЭтоЗаявлениеТолькоНаПереход);
		
		Город = КонтекстЭДОСервер.ПолеСертификата_2_5_4_7(Объект.АдресЮридический);
		ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.Город,Город,,,,,ЭтоЗаявлениеТолькоНаПереход);
		
		Улица = КонтекстЭДОСервер.ПолеСертификата_2_5_4_9(Объект.АдресЮридический);
		ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.Улица,Улица,,,,,ЭтоЗаявлениеТолькоНаПереход);
	...
	ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПФИО, ФИОВладельцаЭЦП,,,,,ЭтоЗаявлениеТолькоНаПереход);
	...
	Если Объект.ТипОрганизации Тогда
		ДобавитьВТаблицу(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПИНН, Объект.ВладелецЭЦПИНН,,,,,ЭтоЗаявлениеТолькоНаПереход);
	КонецЕсли;
