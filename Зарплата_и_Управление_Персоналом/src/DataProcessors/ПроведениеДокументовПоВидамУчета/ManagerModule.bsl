...
Функция МассивДокументов(Параметры, ВидыУчета)
	...
	Если ВидыУчета = "ДанныеДляРасчетаСреднего" Тогда
		...
		|ГДЕ
		|	НачисленияУдержанияПоСотрудникамОбороты.Период МЕЖДУ НачисленияУдержанияПоСотрудникамОбороты.ДатаНачала И &ДатаОкончания
		|	И НачисленияУдержанияПоСотрудникамОбороты.Организация = &Организация
		|	И НачисленияУдержанияПоСотрудникамОбороты.НачислениеУдержание В
		|			(ВЫБРАТЬ
		|				ВТ_НачисленияБезДублей.Начисление
		|			ИЗ
		|				ВТ_НачисленияБезДублей КАК ВТ_НачисленияБезДублей)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
