...
Процедура ИнициализироватьРасчетБазыНачислений(ОписаниеНабораРассчитываемыхЗаписей, УстанавливаемыйМенеджерРасчетаЗарплаты, НастройкиРасчета, ДанныеТекущегоНабораНачислений = Неопределено) Экспорт
	...
	ИнициализироватьРасчетБазы(
		...
		НастройкиРасчета.РасчетПоДаннымТекущегоНабора,
...
Функция РасчетнаяБазаНачислений(ОчередностьРасчета = Неопределено, РезультатыРасчетаБазовыхНачислений = Неопределено) Экспорт	
	...
	Для Каждого РассчитываемаяЗапись Из РассчитываемыйНабор Цикл
		...
		Для Каждого БазоваяЗапись Из БазовыеЗаписи Цикл
			Если ЗависимостиРассчитываемогоВР[БазоваяЗапись.ВидРасчета] <> Неопределено  Тогда
...
Процедура ИнициализироватьРасчетБазы(ОписаниеНабораРассчитываемыхЗаписей, ИспользуемыйАлгоритмРасчета, ПолучатьПодробнуюРасшифровку, РасчетПоДаннымТекущегоНабора, ИсключаемыйРегистратор = Неопределено, ДанныеТекущегоНабораНачислений = Неопределено) 
...
Функция ДанныеБазовыхНачислений(ОписаниеНабораРассчитываемыхЗаписей, АлгоритмОтбораБазовыхЗаписей, РасчетПоДаннымТекущегоНабора, ВидыРасчетаСПодробнойРасшифровкой, ПолучатьРасшифровкуБезусловно = Ложь, ДанныеТекущегоНабораНачислений = Неопределено, ИсключаемыйРегистратор = Неопределено)		
...
Процедура ЗаполнитьБазовыеЗаписиНачисленийПоТекущемуНабору(ОписаниеНабораРассчитываемыхЗаписей, БазовыеЗаписи, ТаблицаНабораНачислений, АлгоритмОтбораНачислений, ВидыРасчетаСПодробнойРасшифровкой)
	...
	Для Каждого СтрокаРассчитываемогоНабора Из РассчитываемыйНабор Цикл
		...
		Для Каждого СтрокаТекущегоНабора Из ЗаписиТекущегоНабора Цикл
			...
			Если АлгоритмОтбораНачислений = ДоступныеАлгоритмы.БазаНачислений Тогда
...
Функция ЗаписьСоответствуетОтборуБазыУдержаний(РассчитываемаяЗапись, БазоваяЗапись, ЗависимостиВидовРасчета)
	...
	Если ЗначениеЗаполнено(БазоваяЗапись.СторноТекущегоПериода) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ДанныеВедущихЗаписейКаскадногоНормирования(ДанныеБазовыхНачислений, ДанныеТекущегоНабораНачислений = Неопределено, РасчетПоДаннымТекущегоНабора = Ложь, ИсключаемыйРегистратор = Неопределено)	
	...
	ВедущиеЗаписиКаскадногоНормирования = ВедущиеЗаписиКаскадногоНормирования(ОтборВедущихЗаписей, ДанныеТекущегоНабораНачислений, РасчетПоДаннымТекущегоНабора, ИсключаемыйРегистратор);
...
Функция ВедущиеЗаписиКаскадногоНормирования(ОтборНормируемыхЗаписей, ДанныеТекущегоНабораНачислений = Неопределено, РасчетПоДаннымТекущегоНабора = Ложь, ИсключаемыйРегистратор = Неопределено)
	Если Не РасчетПоДаннымТекущегоНабора Тогда
