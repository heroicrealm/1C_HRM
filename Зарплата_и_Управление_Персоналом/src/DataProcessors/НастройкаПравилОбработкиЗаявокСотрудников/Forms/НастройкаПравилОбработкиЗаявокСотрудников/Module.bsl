...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПодписыватьЗаявкиСотрудника = Константы.ПодписыватьЗаявкиСотрудника.Получить();
	...
	Если ВидНастройки <> 0 И Не СтруктураПредприятияДоступна Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ИспользуетсяКадровыйЭДОЗаписанаКонстанта" Тогда
		Элементы.ПодписыватьЗаявкиСотрудника.Видимость = Параметр;
		ПодписыватьЗаявкиСотрудника = ?(Не Параметр, Ложь, ПодписыватьЗаявкиСотрудника);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура НастройкиПравилСтруктураПредприятияСрокЗадачиПриИзменении(Элемент)
	ЗаполнитьПодчиненныеЭлементы("СрокЗадачи");
КонецПроцедуры
...
&НаКлиенте
Процедура НастройкиПравилСрокЗадачиПриИзменении(Элемент)
	ЗаполнитьИзНастроекБезСтруктуры("СрокЗадачи");
КонецПроцедуры
...
&НаКлиенте
Процедура ПодписыватьЗаявкиСотрудникаПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура ЗаполнитьНастройкиМаршрутов()
	...
	ИспользуемаяФункциональностьСервиса = РегистрыСведений.ИспользуемаяФункциональностьСервисаКабинетСотрудника.НастройкиИспользуемаяФункциональность();
	...
	Для каждого Роль Из СписокРолей Цикл
		...
		Колонки.Добавить("СрокЗадачи");
		Колонки.Добавить("СписокПользователей", Новый ОписаниеТипов("СписокЗначений"));
		
		Подразделения = ТаблицаПравилНастроек.Строки.НайтиСтроки(Новый Структура("РольИсполнителя", Роль));
		Для каждого Подразделение Из Подразделения Цикл
			НовоеПодразделение = НастройкаПравил.СтруктураПредприятия.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(НовоеПодразделение, Подразделение);
			ЗаполнитьДерево(НовоеПодразделение, Подразделение);
		КонецЦикла;
				
		ОтборПользователей = Новый Структура("РольИсполнителя, Подразделение");
				
		Для каждого ЭлементСтруктурыПредприятия Из ПолучитьЭлементы(НастройкаПравил.СтруктураПредприятия) Цикл
			ОтборПользователей.РольИсполнителя = Роль;
			ОтборПользователей.Подразделение = ЭлементСтруктурыПредприятия.Подразделение;
			...
			ЭлементСтруктурыПредприятия.СписокПользователей.ЗагрузитьЗначения(
				ТаблицаПользователей.Скопировать(ОтборПользователей).ВыгрузитьКолонку("Исполнитель"));
		КонецЦикла;
			
		Если Подразделения.Количество() > 0 Тогда
			Подразделение = НастройкаПравил.СтруктураПредприятия.Строки[0];
			
			НастройкаПравил.СписокПользователей.ЗагрузитьЗначения(Подразделение.СписокПользователей.ВыгрузитьЗначения());
			НастройкаПравил.СрокЗадачи = Подразделение.СрокЗадачи
		КонецЕсли;
	КонецЦикла;
...
Функция ТаблицаПравилНастроек(СписокРолей)
	...
	Если ПубликоватьСтруктуруЮридическихЛиц Тогда
		...
		|			ПО ИсполнителиЗадач.РольИсполнителя = ПравилаОбработкиЗаявокСотрудников.РольИсполнителя
		|				И (ПравилаОбработкиЗаявокСотрудников.Подразделение В (&ПустыеПодразделения)
		|					ИЛИ ИсполнителиЗадач.ОсновнойОбъектАдресации = ПравилаОбработкиЗаявокСотрудников.Подразделение)
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПравилаОбработкиЗаявокСотрудников КАК ПравилаОбработкиЗаявокСотрудников1
		|			ПО ИсполнителиЗадач.РольИсполнителя = ПравилаОбработкиЗаявокСотрудников1.РольИсполнителя
		|				И ИсполнителиЗадач.ОсновнойОбъектАдресации = ПравилаОбработкиЗаявокСотрудников1.Подразделение
		...
		|					ИЛИ ИсполнителиЗадач.РольИсполнителя = ПравилаОбработкиЗаявокСотрудников2.РольИсполнителя)
		|				И (ПравилаОбработкиЗаявокСотрудников2.Подразделение В (&ПустыеПодразделения)
		|					ИЛИ ИсполнителиЗадач.ОсновнойОбъектАдресации = ПравилаОбработкиЗаявокСотрудников2.Подразделение)
	...
	Иначе
		...
		|ПОМЕСТИТЬ ВТПодразделенияРоли
		|ИЗ
		|	#Таблица КАК СтруктураПредприятия
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РолиИсполнителей КАК РолиИсполнителей
		|		ПО (РолиИсполнителей.Ссылка В (&РолиИсполнителей))
		|;
		|
...
Процедура ЗаполнитьДерево(Приемник, Источник)
	...
	Для каждого ЭлементИсточника Из Источник.Строки Цикл
		...
		Если ЭлементИсточника.Строки.Количество() > 0 Тогда
			ЗаполнитьДерево(ЭлементПриемника, ЭлементИсточника);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.НастройкиПравилСрокЗадачи.Видимость = НастройкиПоВидамЗаявки;
	...
	Элементы.ПодписыватьЗаявкиСотрудника.Видимость = ПолучитьФункциональнуюОпцию("ИспользуетсяКадровыйЭДОКабинетСотрудника");
...
Процедура ЗаполнитьНастройкиБезСтруктурыДляРоли(Роль)
	...
	НастройкаПравилОбработки.СрокЗадачи = НастройкаПравилОбработкиСтруктураПредприятия.СрокЗадачи;
...
Процедура ВидНастройкиПриИзмененииНаСервере()
	...
	ЗаполнитьИзНастроекБезСтруктурыНаСервере("СрокЗадачи");
