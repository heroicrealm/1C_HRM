...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2019_1(Данные, УникальныйИдентификатор)
	СпискиВыбора = УведомлениеОСпецрежимахНалогообложения.СформироватьСпискиВыбораДляПроверкиВыгрузки(
		Данные.Объект.ИмяОтчета, "СпискиВыбора2019_1");
	...
	УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
		Титульная, СпискиВыбора, "СпособИнфРез", "Титульная", Истина,
		"способ информирования налогоплательщика о результатах рассмотрения настоящего заявления", ТаблицаОшибок);
	...
	Для Каждого Лист0Итер Из Данные.ДанныеМногостраничныхРазделов.Транспорт Цикл
		...
		Для Каждого Лист1Итер Из Данные.ДанныеМногостраничныхРазделов.ЛьготаДокументТранспорт Цикл
			...
			КонецЕсли;
			УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
				Лист1, СпискиВыбора, "ПрПериодДок", "ЛьготаДокументТранспорт", Истина, "период действия документа", ТаблицаОшибок);
			...
			Если Не ЗначениеЗаполнено(Лист1.НомерДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не заполнен номер документа", "ЛьготаДокументТранспорт", "НомерДок", Лист1.УИД));
			КонецЕсли;
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
			Лист0, СпискиВыбора, "ВидТС", "Транспорт", Истина, "вид транспортного средства", ТаблицаОшибок);
		УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
			Лист0, СпискиВыбора, "КодНалЛьготТр", "Транспорт", Истина, "код налоговой льготы", ТаблицаОшибок);
		...
		Если "30200" = Лист0.КодНалЛьготТр Тогда
			Если ЗначениеЗаполнено(Лист0.ДатаЗакон) Или ЗначениеЗаполнено(Лист0.НомерЗакон) Или ЗначениеЗаполнено(Лист0.СтруктЕдЗакон) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"При коде льготы 30200 сведения о законе не заполняются", "Транспорт", "ДатаЗакон", Лист0.УИД));
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Лист0.ДатаЗакон) Или ЗначениеЗаполнено(Лист0.НомерЗакон) Или ЗначениеЗаполнено(Лист0.СтруктЕдЗакон) Тогда 
	...
	Для Каждого Лист0Итер Из Данные.ДанныеМногостраничныхРазделов.Земля Цикл
		...
		Для Каждого Лист1Итер Из Данные.ДанныеМногостраничныхРазделов.ЛьготаДокументЗемля Цикл
			...
			КонецЕсли;
			УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
				Лист1, СпискиВыбора, "ПрПериодДок", "ЛьготаДокументЗемля", Истина, "период действия документа", ТаблицаОшибок);
			Если Не ЗначениеЗаполнено(Лист1.ДатаНачПериод) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			...
			Если Не ЗначениеЗаполнено(Лист1.НомерДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не заполнен номер документа", "ЛьготаДокументЗемля", "НомерДок", Лист1.УИД));
			КонецЕсли;
		...
		УведомлениеОСпецрежимахНалогообложения.ПроверитьПоказательНаСоотвествиеСписку(
			Лист0, СпискиВыбора, "КодНалЛьготЗем", "Земля", Истина, "код налоговой льготы", ТаблицаОшибок);
...
Функция СформироватьСписокЛистовФорма2019_1(Объект) Экспорт
	...
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(
		"РегламентированноеУведомлениеЛьготаТранспортЗемля", СтруктураПараметров.ДанныеУведомления["Титульная"],
