...
Процедура _РеестрДокПОбНЛКодОперНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если (ЗначениеЗаполнено(_ОтчетГод) И ОТЧ.ПривестиЗначение(_ОтчетГод) <= 2021)
		Или (ОТЧ.ПривестиЗначение(_ОтчетГод) = 2022 И (_Период = "21" Или _Период = "51")) Тогда 
...
Процедура _РеестрДокПОбНЛПриАктивизацииСтроки(Элемент)
	...
	Если Элементы._РеестрДокПОбНЛ.ТекущиеДанные <> Неопределено
		...
		Если Счетчик < 1000 И Элементы._ПредТипДогТек.Видимость = Истина Тогда 
...
Процедура ВывестиТекущееДерево()
	...
	СпозиционироватьсяНаОшибочнойСтроке(ПредТипДогТек);
	ОбщегоНазначенияКлиент.СообщитьПользователю("Ошибочные данные подсвечены красным цветом");
КонецПроцедуры

&НаКлиенте
Процедура СпозиционироватьсяНаОшибочнойСтроке(ТекущийЭлемент)
	Для Каждого Элемент Из ТекущийЭлемент.ПолучитьЭлементы() Цикл 
		...
		СпозиционироватьсяНаОшибочнойСтроке(Элемент);
	КонецЦикла;
КонецПроцедуры
