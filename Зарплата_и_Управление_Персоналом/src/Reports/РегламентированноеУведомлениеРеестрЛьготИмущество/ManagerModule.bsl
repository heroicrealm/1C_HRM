...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2020_1(Данные, УникальныйИдентификатор)
	...
	ИначеЕсли Не УведомлениеОСпецрежимахНалогообложения.СтрокаСодержитТолькоЦифры(Титульная.КодНО) 
	...
	Иначе
		...
		КонецЕсли;
		Если Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(Титульная.КПП, "")
			Или СтрНайти("01|51|52|55|56|63|64|65|91|92|70|4A|4B|4C|4D|4E|4F|", Сред(Титульная.КПП, 5, 2) + "|") = 0 Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
	...
	Если ЗначениеЗаполнено(Титульная.ФормРеорг)
		...
		Если Титульная.ФормРеорг = "1" Или Титульная.ФормРеорг = "2" Или Титульная.ФормРеорг = "3" 
			...
			Если Не ЗначениеЗаполнено(Титульная.ИННРеорг)
				...
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указан / неправильно указан ИНН реорганизованной организации", "Титульная", "ИННРеорг"));
			КонецЕсли;
			...
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Титульная.ИННРеорг)
				И Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(Титульная.ИННРеорг, Истина, "") Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Неправильно указан ИНН реорганизованной организации", "Титульная", "ИННРеорг"));
			КонецЕсли;
...
Функция СформироватьСписокЛистовФорма2020_1(Объект) Экспорт
	...
	МакетПФ = Отчеты.РегламентированноеУведомлениеРеестрЛьготИмущество.ПолучитьМакет("Печать_MXL_Форма2020_1");
