...
Функция ПолучитьМакетТабличногоДокумента(Форма, ИмяМакета)
	Возврат УведомлениеОСпецрежимахНалогообложения.ЗагрузитьМакетыИзАрхива(Форма, "", "ЭкранныеФормы21")[ИмяМакета];
КонецФункции
	...
	Для Каждого Стр Из ПараметрыИнициализацииФормы.ДеревоСтраниц.Строки Цикл
		Если Стр.Многостраничность Тогда 
			Если Стр.Многострочность Тогда
				...
				МакетДокумента = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, МнгСтр.ИмяМакета).ПолучитьОбласть("ОсновнаяЧасть");
				...
				Для Каждого МногострочныйЭлемент Из МнгСтр.МногострочныеЧасти Цикл
					...
					МакетМногострочки = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, МнгСтр.ИмяМакета).ПолучитьОбласть("Str_"+МногострочныйЭлемент.Значение);
			...
			Иначе
				...
				СтруктураМногостраничнойСтраницы = Новый Структура("УИД", МнгСтр.УИД);
				МакетДокумента = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, МнгСтр.ИмяМакета);
				Для Каждого Обл Из МакетДокумента.Области Цикл 
					Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
						И Обл.СодержитЗначение Тогда 
		...
		Иначе
			...
			Если Стр.Многострочность Тогда
				...
				МакетДокумента = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, Стр.ИмяМакета).ПолучитьОбласть("ОсновнаяЧасть");
				Для Каждого Обл Из МакетДокумента.Области Цикл 
					Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
						И Обл.СодержитЗначение Тогда 
				...
				Для Каждого МногострочныйЭлемент Из Стр.МногострочныеЧасти Цикл
					...
					ТЗ.Колонки.Добавить("УИД");
					СтрокаДанных = Новый Структура;
					МакетМногострочки = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, Стр.ИмяМакета).ПолучитьОбласть("Str_"+МногострочныйЭлемент.Значение);
					Для Каждого Обл Из МакетМногострочки.Области Цикл 
						Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
							И Обл.СодержитЗначение Тогда 
		...
		Если ЗначениеЗаполнено(ПодчМнг.ИДНаименования) Тогда
			Если ПодчМнг.Многострочность Тогда
				Для Каждого МногострочныйЭлемент Из ПодчМнг.МногострочныеЧасти Цикл
					...
					ТЗ.Колонки.Добавить("УИД");
					СтрокаДанных = Новый Структура;
					МакетМногострочки = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, ПодчМнг.ИмяМакета).ПолучитьОбласть("Str_"+МногострочныйЭлемент.Значение);
					Для Каждого Обл Из МакетМногострочки.Области Цикл 
						Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
							И Обл.СодержитЗначение Тогда 
			...
			МакетДокумента = ПолучитьМакетТабличногоДокумента(ПараметрыИнициализацииФормы, ПодчМнг.ИмяМакета);
			Для Каждого Обл Из МакетДокумента.Области Цикл 
				Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
					И Обл.СодержитЗначение Тогда 
