	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил3" Тогда
		...
		Пока Обл <> Неопределено Цикл
			...
			Если СокрЛП(Обл.Значение) = "2" Тогда
				ЗапретитьВводДанныхВОбласть(Форма, Форма.ТабличныйДокумент.Область("П00013М106002" + Постфикс));
				ЗапретитьВводДанныхВОбласть(Форма, Форма.ТабличныйДокумент.Область("П00013М106003" + Постфикс));
			Иначе
				РазрешитьВводДанныхВОбласть(Форма, Форма.ТабличныйДокумент.Область("П00013М106002" + Постфикс));
				РазрешитьВводДанныхВОбласть(Форма, Форма.ТабличныйДокумент.Область("П00013М106003" + Постфикс));
			КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил3" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если СтрЧислоВхождений(СтруктЗависимыхЯчеек["П00013М104002"], ОбластьИмяБезИнд) > 0
					ИЛИ СтрЧислоВхождений(СтруктЗависимыхЯчеек["П00013М104003"], ОбластьИмяБезИнд) > 0 Тогда
					...
					Пока Обл <> Неопределено Цикл
						Знаменатель = ТДО["П00013М102002" + Инд_Строка].Значение;
						ТДО["П00013М104002" + Инд_Строка].Значение = ?(Знаменатель = 0,
						0, ТДО["П00013М103002" + Инд_Строка].Значение / Знаменатель * 100);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил3" Тогда
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			ДанСтр = СтрокаМнЧ.Данные;
			Знаменатель = ДанСтр["П00013М102002"];
			ДанСтр.Вставить("П00013М104002", ?(Знаменатель = 0, 0, ДанСтр["П00013М103002"] / Знаменатель * 100));
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	//
	Структ = Новый Структура;
	Структ.Вставить("П00013М104002", "П00013М102002;П00013М103002");
...
Процедура Проверить(Команда)
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 2022 Тогда
		...
		"ru='Проверка контрольных соотношений для 2023 и более поздних 
