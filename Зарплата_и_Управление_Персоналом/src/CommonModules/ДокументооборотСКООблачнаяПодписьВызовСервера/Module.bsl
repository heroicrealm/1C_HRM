...
Функция ПодготовитьСертификат(ДанныеСертификата, ДанныеПодписи = Неопределено) Экспорт
	...
	Результат = Ложь;
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяОблачнаяПодпись")
		И НЕ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодписьСервисаDSS") Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ПодготовитьСертификатСПараметрами(ПараметрыОперации, АдресРезультата = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяОблачнаяПодпись")
		...
		МодульСервисКриптографииDSSASNКлиентСервер = ОбщегоНазначения.ОбщийМодуль("СервисКриптографииDSSКлиентСервер");
		Сертификат = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(ПараметрыОперации, "Сертификат");
		Подпись = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(ПараметрыОперации, "Подпись");
...
Функция ПроверитьПодписьГОСТ2012(ИсходныеДанные, ДанныеПодписи, СертификатПроверки = Неопределено) Экспорт
	...
	Если СертификатПроверки <> Неопределено Тогда
		СвойстваСертификата		= СервисКриптографииDSSASNКлиентСервер.ПолучитьСвойстваСертификата(СертификатПроверки);
	...
	Иначе
		СвойстваПодписи = СервисКриптографииDSSASNКлиентСервер.ПолучитьСвойстваПодписи(ДанныеПодписи);
...
Функция ПолучитьДанныеСертификатаКО()
	...
	// необходимо заменить содержимое
	ИсходныеДаннные = 
	"MIIFNjCCBOOgAwIBAgIQfO+bAA+sCpxJ8eKW6794YTAKBggqhQMHAQEDAjCB4DEg
	|MB4GCSqGSIb3DQEJARYRaW5mb0BjcnlwdG9wcm8ucnUxGDAWBgUqhQNkARINMTAz
	|NzcwMDA4NTQ0NDEaMBgGCCqFAwOBAwEBEgwwMDc3MTcxMDc5OTExCzAJBgNVBAYT
	|AlJVMRUwEwYDVQQHDAzQnNC+0YHQutCy0LAxJTAjBgNVBAoMHNCe0J7QniAi0JrQ
	|oNCY0J/QotCeLdCf0KDQniIxOzA5BgNVBAMMMtCi0LXRgdGC0L7QstGL0Lkg0KPQ
	|piDQtNC70Y8g0YHRgtC10L3QtNCwIERTUyAyMDEyMB4XDTIwMDgwNjA5MTc0NVoX
	|DTIxMTEwNjA5Mjc0NVowUTELMAkGA1UEBhMCUlUxETAPBgNVBCoMCNCf0LXRgtGA
	|MRswGQYDVQQEDBLQn9GA0LjRgtC+0YDQvdC+0LIxEjAQBgNVBAMMCVRlc3ROZXcg
	|MTBmMB8GCCqFAwcBAQEBMBMGByqFAwICJAAGCCqFAwcBAQICA0MABEC3/aphoYGF
	|hnqL3nK5eU1wkR3GF/z1kVIKTIT5rpFOMcTkVHgQtYELBEUg4JwHpk5oHiVDEQ9L
	|KUSYhKAoPxYoo4IC/jCCAvowDgYDVR0PAQH/BAQDAgPoMB0GA1UdDgQWBBQo3HGr
	|uKFmhMYt81xXodiZ/4HMwzA0BgkrBgEEAYI3FQcEJzAlBh0qhQMCAjIBCYHC/TiE
	|kPp5hdWdG4GI3jKBok7uSwIBAQIBADAdBgNVHSUEFjAUBggrBgEFBQcDAgYIKwYB
	|BQUHAwQwgYcGCCsGAQUFBwEBBHsweTArBggrBgEFBQcwAYYfaHR0cDovL2Rzc2xh
	|Yi9vY3NwMjAxMi9vY3NwLnNyZjBKBggrBgEFBQcwAoY+aHR0cDovL2Rzc2xhYi9h
	|aWEvNTIyYzYxNzU0YWQyZjYxNGQ4NjhiZDIwZmU0NThlNjRiZWM4YTRlYy5jcnQw
	|EwYDVR0gBAwwCjAIBgYqhQNkcQEwKwYDVR0QBCQwIoAPMjAyMDA4MDYwOTE3NDVa
	|gQ8yMDIxMDgwNjA5MTc0NVowNQYFKoUDZG8ELAwq0JrRgNC40L/RgtC+LdCf0YDQ
	|viBDU1AgKNCy0LXRgNGB0LjRjyA0LjApME8GA1UdHwRIMEYwRKBCoECGPmh0dHA6
	|Ly9kc3NsYWIvY2RwLzUyMmM2MTc1NGFkMmY2MTRkODY4YmQyMGZlNDU4ZTY0YmVj
	|OGE0ZWMuY3JsMIIBHgYDVR0jBIIBFTCCARGAFFIsYXVK0vYU2Gi9IP5FjmS+yKTs
	|oYHmpIHjMIHgMSAwHgYJKoZIhvcNAQkBFhFpbmZvQGNyeXB0b3Byby5ydTEYMBYG
	|BSqFA2QBEg0xMDM3NzAwMDg1NDQ0MRowGAYIKoUDA4EDAQESDDAwNzcxNzEwNzk5
	|MTELMAkGA1UEBhMCUlUxFTATBgNVBAcMDNCc0L7RgdC60LLQsDElMCMGA1UECgwc
	|0J7QntCeICLQmtCg0JjQn9Ci0J4t0J/QoNCeIjE7MDkGA1UEAwwy0KLQtdGB0YLQ
	|vtCy0YvQuSDQo9CmINC00LvRjyDRgdGC0LXQvdC00LAgRFNTIDIwMTKCEBFOQQGB
	|qnaOTddxnUl1YjEwCgYIKoUDBwEBAwIDQQCLXyxOxSiIsV1Vr8s2spdhavUZqQ42
	|p9mF5YYBVnq7OYMCAGhCqw8/FXT8YAj+PIExGlk34f64tmyeR+RMyrUp";
