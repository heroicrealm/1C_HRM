...
Процедура ВыполненоВыполнитьЗавершение(Форма, Результат, ДополнительныеПараметры) Экспорт
	...
	БизнесПроцессыЗаявокСотрудниковВызовСервера.ВыполненоВыполнить(Форма.СостояниеЗапроса, Форма.Исполнитель);
...
Процедура ОтказатьЗавершение(Форма, Результат, ДополнительныеПараметры) Экспорт
	...
	Отказ = Ложь;
	БизнесПроцессыЗаявокСотрудниковВызовСервера.Отказать(Форма.СостояниеЗапроса, Форма.Исполнитель);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
