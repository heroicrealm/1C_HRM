	...
	Для Каждого Результат Из ТаблицаСобытий Цикл
		...
		Для Каждого СобытиеОтпусков Из Результат.БудущиеОтпуска Цикл
			...
			Если ЗначениеЗаполнено(СобытиеОтпусков.ФактическиеОтпуска) Тогда
				...
				Для Каждого ВидОтпуска Из ВидыОтпусковСотрудника Цикл
					...
					ИспользованоДней = ?(ИспользованоДней = Неопределено, КоличествоДней, ИспользованоДней + КоличествоДней);
		...
		Если Результат.БудущиеОтпуска.Количество() > 0 Тогда
			...
			Для Каждого ВидОтпуска Из ВидыОтпусковСотрудника Цикл
				...
				ОбластьМакета.Параметры.КоличествоДней = ?(КоличествоДней - ИспользованоДней = 0, "0,00", Формат(КоличествоДней - ИспользованоДней, ФорматЧисла));
				ТабДокумент.Присоединить(ОбластьМакета);
			КонецЦикла;
		КонецЕсли;
...
Функция ИнформацияОбОтпусках(Сотрудники, ТекущиеОстаткиОтпусков)
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущиеОстатки.Сотрудник КАК Сотрудник,
		|	ТекущиеОстатки.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ТекущиеОстатки.КоличествоДней КАК КоличествоДней,
		|	ТекущиеОстатки.ДнейКомпенсации КАК ДнейКомпенсации,
		|	ТекущиеОстатки.ДопустимаяПродолжительностьОтпуска КАК ДопустимаяПродолжительностьОтпуска,
		|	ВЫБОР
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Приоритет
		|ПОМЕСТИТЬ ВТТекущиеОстаткиПриоритет
		|ИЗ
		|	ВТТекущиеОстаткиОтпусковПредварительно КАК ТекущиеОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		|		ПО ТекущиеОстатки.Сотрудник = КадровыеДанные.Сотрудник
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущиеОстаткиПриоритет.ФизическоеЛицо КАК ФизическоеЛицо,
		|	МАКСИМУМ(ТекущиеОстаткиПриоритет.Приоритет) КАК Приоритет
		|ПОМЕСТИТЬ ВТТекущиеОстаткиМаксимальныйПриоритет
		|ИЗ
		|	ВТТекущиеОстаткиПриоритет КАК ТекущиеОстаткиПриоритет
		|
		|СГРУППИРОВАТЬ ПО
		|	ТекущиеОстаткиПриоритет.ФизическоеЛицо
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущиеОстаткиПриоритет.ФизическоеЛицо КАК ФизическоеЛицо,
		|	МАКСИМУМ(ТекущиеОстаткиПриоритет.КоличествоДней) КАК КоличествоДней,
		|	МАКСИМУМ(ТекущиеОстаткиПриоритет.ДнейКомпенсации) КАК ДнейКомпенсации,
		|	МАКСИМУМ(ТекущиеОстаткиПриоритет.ДопустимаяПродолжительностьОтпуска) КАК ДопустимаяПродолжительностьОтпуска
		|ПОМЕСТИТЬ ВТТекущиеОстаткиОтпусков
		|ИЗ
		|	ВТТекущиеОстаткиПриоритет КАК ТекущиеОстаткиПриоритет
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТекущиеОстаткиМаксимальныйПриоритет КАК МаксимальныйПриоритет
		|		ПО ТекущиеОстаткиПриоритет.ФизическоеЛицо = МаксимальныйПриоритет.ФизическоеЛицо
		|			И ТекущиеОстаткиПриоритет.Приоритет = МаксимальныйПриоритет.Приоритет
		|
		|СГРУППИРОВАТЬ ПО
		|	ТекущиеОстаткиПриоритет.ФизическоеЛицо
		|;
