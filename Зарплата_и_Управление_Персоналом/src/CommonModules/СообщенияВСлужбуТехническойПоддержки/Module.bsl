...
//
Функция ШаблонТекстаСообщения(ДанныеСообщения)
	
	Добавлять = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеСообщения, "ИспользоватьСтантартныйШаблон");
	Сообщение = ДанныеСообщения.Сообщение;
...
Функция ОперацияОтправкиДанных(
	...
	ДанныеJSON = ПараметрыSaveTechSupportRequestJSON(
		...
		ШаблонТекстаСообщения(ДанныеСообщения),
