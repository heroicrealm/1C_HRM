...
Функция ПодготовитьФайлы(ДокументЗаявление)
	...
	Для Каждого ЭлектронныйДокумент Из ДокументЗаявление.ЭлектронныеДокументы Цикл
		...
		document = ОбработкаЗаявленийАбонентаКлиентСервер.ВидДокументаЗаявленияСкорректированный(ЭлектронныйДокумент.Документ);
...
Функция СертификатДолженБытьПолученОтГосУЦ(Форма) Экспорт
	...
	СертификатДолженБытьПолученОтГосУЦ = 
		Форма.ЭтоПолучениеЭПС2022Г
		И Форма.ЭтоБюджетополучатель;
...
Функция ЭтоПолучениеЭПС2022Г(Дата) Экспорт
	...
	Возврат ОбработкаЗаявленийАбонентаКлиентСервер.ЭтоПолучениеЭПС2022Г(Дата);
...
Функция УЦВЗаявлении(ДокументЗаявление) Экспорт
	...
	Если ОбработкаЗаявленийАбонентаКлиентСервер.ЭтоПолучениеСертификатВКА(ДокументЗаявление) Тогда
		КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
		УЦСтрокой = КонтекстЭДОСервер.ДанныеУЦ(ДокументЗаявление).НаименованиеУЦ;
	Иначе
		Если УЦ = Перечисления.УдостоверяющиеЦентрыБРО.УЦФНС Тогда
			УЦСтрокой = НСтр("ru = 'Федеральная налоговая служба'");
		ИначеЕсли УЦ = Перечисления.УдостоверяющиеЦентрыБРО.УЦАналитическийЦентр Тогда
...
Процедура ИзменитьОформлениеПодсказкиСУЦ(Форма)
	...
	Если Форма.ЭтоПолучениеЭПС2022Г И Форма.ЭтоБюджетополучатель Тогда
	...
	ИначеЕсли Форма.ЭтоПолучениеЭПС2022Г И ЭтоРуководитель И НЕ Форма.ВозможноБесшовноеПолучениеСертификатаВДУЦ Тогда
