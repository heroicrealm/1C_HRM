...
Процедура СоздатьИнформационнуюСистемуФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Если Не Отказ Тогда
		...
		КабинетСотрудника.ОбновитьНастройкиФункциональностиСервиса(Ложь);
	КонецЕсли;
...
Процедура ИнформацияОСервисеФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Ответ = ИнформацияОПриложении(Настройки.ИдентификаторПриложения, Ложь);
	Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
		Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
		Отказ = Истина;
	Иначе
...
Процедура ЗавершитьСценарийОтложенногоПодключенияФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	АктивироватьТестовыйТариф(Ложь);
...
Процедура ОтправитьСообщениеОбАктивацииТестовогоТарифа(ВестиПротокол) Экспорт
	...
	СообщениеОПодключенииТестовогоТарифа(ВестиПротокол);
...
Функция ВерсияПриложения(ВестиПротокол) Экспорт
	...
	Настройки = РегистрыСведений.НастройкиСервисаКабинетСотрудника.НастройкиСервиса();
	Ответ = ИнформацияОПриложении(Настройки.ИдентификаторПриложения, Ложь, ВестиПротокол);
	Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
...
// 			* СообщениеОбОшибке - Строка, Неопределено - сообщение об ошибке.
//
Функция ИнформацияОПриложении(ИдентификаторПриложения, РегистрироватьОшибкуСозданияПриложения, ВестиПротокол = Ложь)
...
Процедура СообщениеОПодключенииТестовогоТарифа(ВестиПротокол)
...
Процедура АктивироватьТестовыйТариф(ВестиПротокол) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		КонецЕсли;
	Иначе
		ПодключитьТестовыйПериод(ВестиПротокол);
	КонецЕсли;
...
Процедура НачатьПодключениеКСервисуВОтложенномРежиме(ПараметрыСоздания) Экспорт
...
Процедура ПодключитьТестовыйПериод(ВестиПротокол)
