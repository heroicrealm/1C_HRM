...
Процедура ВыполнитьРаспределениеУдержаний(РаспределениеУдержаний, ИсходныеДанные) Экспорт
	...
	|	ЕСТЬNULL(УсловияУдержанияПоИсполнительномуДокументу.УчитыватьБольничныеЛисты, ЛОЖЬ)
	|		ИЛИ ЕСТЬNULL(УсловияУдержанияПоИсполнительномуДокументу.ВидБазы, ЗНАЧЕНИЕ(Перечисление.ВидыБазыУдержанияПоИсполнительномуДокументу.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыБазыУдержанияПоИсполнительномуДокументу.ПрожиточныйМинимум) КАК УчитыватьБольничныеЛисты
	|ИЗ
