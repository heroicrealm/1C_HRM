...
Процедура РассчитатьИЗарегистрироватьУдержанныеНалоги(Ссылка, Движения, Отказ, Организация, ДатаОперации, МенеджерТаблиц, Записывать = Ложь, ПериодРегистрации = Неопределено, ОкончательныйРасчет = Истина, РегистрироватьПеречислениеНДФЛ = Истина, ДатаПолученияДохода = Неопределено) Экспорт
	...
	Если УдержанныйНалог.Количество() = 0 Тогда
		Возврат
	КонецЕсли;
	
	Если ДатаПолученияДохода = Неопределено Тогда
		ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(МенеджерТаблиц, УдержанныйНалог, "ВТНалогУдержанный");
	Иначе
		УчетФактическиПолученныхДоходов.СоздатьВТНалогУдержанный(МенеджерТаблиц, УдержанныйНалог, ДатаПолученияДохода)
	КонецЕсли;
...
Процедура ОпределитьПроверкиВеденияУчета(ГруппыПроверок, Проверки) Экспорт
	...
	ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ = ГруппыПроверок.Добавить();
	ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Наименование					= НСтр("ru = 'Учет по НДФЛ'");
	ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор					= "КонтрольВеденияУчетаПоНДФЛ";
	Если ИдентификаторГруппыПроверокВеденияУчетаБЗК <> Неопределено Тогда
		ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.ИдентификаторГруппы = ИдентификаторГруппыПроверокВеденияУчетаБЗК;
	КонецЕсли;
	ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.КонтекстПроверокВеденияУчета	= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор;
	
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор;
	Проверка.Наименование					= НСтр("ru = 'Коды доходов'");
	Проверка.Причины						= НСтр("ru = 'Потеря связи со справочником кодов дохода'");
	Проверка.Рекомендация					= НСтр("ru = 'Указать правильные коды доходов'");
	Проверка.Идентификатор					= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор + ".ПроверкаЦелостностиУчета";
	Проверка.ОбработчикПроверки				= "УчетНДФЛ.ПроверитьКорректностьКодовДохода";
	Проверка.КонтекстПроверокВеденияУчета	= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор;
	
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы			= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор;
	Проверка.Наименование					= НСтр("ru = 'Коды вычетов'");
	Проверка.Причины						= НСтр("ru = 'Потеря связи со справочником кодов вычетов'");
	Проверка.Рекомендация					= НСтр("ru = 'Указать правильные коды вычетов'");
	Проверка.Идентификатор					= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор + ".ПроверкаЦелостностиУчетаВычетов";
	Проверка.ОбработчикПроверки				= "УчетНДФЛ.ПроверитьКорректностьКодовВычетов";
	Проверка.КонтекстПроверокВеденияУчета	= ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Идентификатор;
	
КонецПроцедуры

Процедура ПроверитьКорректностьКодовДохода(Проверка, ПараметрыПроверки) Экспорт
	...
	Результаты = РезультатПроверкиКорректностиКодовДохода();
	
	Если Не Результаты.Документы.Пустой() Тогда
		
		ТаблицаПроблем = Результаты.Документы.Выгрузить();
	...
	Если Не Результаты.Справочники.Пустой() Тогда
		
		ТаблицаПроблем = Результаты.Справочники.Выгрузить();
		Для Каждого СтрокаПроблемы Из ТаблицаПроблем Цикл
			Проблема = МодульКонтрольВеденияУчета.ОписаниеПроблемы(СтрокаПроблемы.Ссылка, ПараметрыПроверки);
			КонтрольВеденияУчетаБЗК.ЗаписатьПроблему(Проблема, ПараметрыПроверки);
		КонецЦикла;
...
Процедура ПроверитьКорректностьКодовВычетов(Проверка, ПараметрыПроверки) Экспорт
	...
	МодульКонтрольВеденияУчета = ОбщегоНазначения.ОбщийМодуль("КонтрольВеденияУчета");
	Результаты = РезультатПроверкиКорректностиКодовВычетов();
	
	Если Не Результаты.Пустой() Тогда
		
		ТаблицаПроблем = Результаты.Выгрузить();
	...
	Для каждого СтрокаТЗ Из Результаты Цикл
		...
		Если НалогНаПрибыльДляДивидендов * НоваяСтрокаРасчетовНалогоплательщика.СуммаСПревышения > 0 Тогда 
			Если НоваяСтрокаРасчетовНалогоплательщика.СуммаСПревышения > 0 Тогда
				ОстатокСуммыКЗачету = Макс(НалогНаПрибыльДляДивидендов - НоваяСтрокаРасчетовНалогоплательщика.СуммаСПревышения, 0);
			...
			Иначе
				ОстатокСуммыКЗачету = Мин(НалогНаПрибыльДляДивидендов - НоваяСтрокаРасчетовНалогоплательщика.СуммаСПревышения, 0);
...
//				ПримененныйВычетВзносыНаДобровольноеСтрахованиеЖизни,ПримененныйВычетРасходыНаЛекарства,ПримененныйВычетРасходыНаФизкультуруИСпорт
...
Процедура СформироватьНДФЛКПеречислению(Движения, Отказ, КоллекцияДвижений = Неопределено) Экспорт
	...
	ЕстьНовыеСтроки = Ложь; УточнятьКрайнийСрокУплаты = Ложь; Организация = Неопределено;
	Для каждого СтрокаДвижений Из НайденныеСтроки Цикл
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(НоваяСтрока.КрайнийСрокУплаты) Тогда
	...
	Если ЕстьНовыеСтроки Тогда
		Если УточнятьКрайнийСрокУплаты Тогда
			ВременнаяТаблица = РегистрыНакопления.РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СоздатьНаборЗаписей().ВыгрузитьКолонки();
			Для каждого СтрокаНабора Из Движения.НДФЛКПеречислению Цикл
				ЗаполнитьЗначенияСвойств(ВременнаяТаблица.Добавить(), СтрокаНабора);
			КонецЦикла; 
			ПроставитьКрайнийСрокУплаты(ВременнаяТаблица, Организация);
			ВсегоСтрок = ВременнаяТаблица.Количество();
			Для НомерСтроки = 0 По ВсегоСтрок - 1 Цикл
				Движения.НДФЛКПеречислению[НомерСтроки].КрайнийСрокУплаты = ВременнаяТаблица[НомерСтроки].КрайнийСрокУплаты
			КонецЦикла;
		КонецЕсли;
...
Процедура ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор, ДатаВыплаты = Неопределено, ДатаОперации = Неопределено) Экспорт
	...
	|	КОНЕЦ КАК СуммаВычета,
	|	ВЫБОР
	|		КОГДА СведенияОДоходахНДФЛ.КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года = ""2012""
	|			ТОГДА СведенияОДоходахНДФЛ.СуммаДохода
	|		КОГДА СведенияОДоходахНДФЛ.КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года = ""2300""
	|			ТОГДА СведенияОДоходахНДФЛ.СуммаДохода
	|		ИНАЧЕ 0
	...
	|	СУММА(Налоги.СуммаСПревышения) КАК СуммаСПревышения
	...
	|	СУММА(НалогиИДоходы.Сумма) КАК Сумма,
	|	СУММА(НалогиИДоходы.СуммаСПревышения) КАК СуммаСПревышения,
	|	СУММА(ВЫБОР
...
Функция РассчитатьУдержанныеНалоги(Ссылка, Организация, ДатаОперации, МенеджерТаблиц, ПериодРегистрации = Неопределено, ОкончательныйРасчет = Истина, ТаблицаВыплат = Неопределено) Экспорт
	...
	Если Не Результат.Пустой() Тогда // Производим зачет сумм налога на прибыль в счет исчисленного налога с дивидендов.
		...
		Для каждого СтрокаТаблицы Из ТаблицаКОбработке Цикл
			...
			Если НалогНаПрибыльДляДивидендов * СтрокаТаблицы.СуммаСПревышения > 0 Тогда
				Если СтрокаТаблицы.СуммаСПревышения > 0 Тогда
					ОстатокСуммыКЗачету = Макс(НалогНаПрибыльДляДивидендов - СтрокаТаблицы.СуммаСПревышения, 0);
					СтрокаТаблицы.СуммаСПревышения = Макс(СтрокаТаблицы.СуммаСПревышения - НалогНаПрибыльДляДивидендов, 0);
				Иначе
					ОстатокСуммыКЗачету = Мин(НалогНаПрибыльДляДивидендов - СтрокаТаблицы.СуммаСПревышения, 0);
					СтрокаТаблицы.СуммаСПревышения = Мин(СтрокаТаблицы.СуммаСПревышения - НалогНаПрибыльДляДивидендов, 0);
				КонецЕсли;
				НалогНаПрибыльДляДивидендов = ОстатокСуммыКЗачету;
			КонецЕсли;
...
Процедура ПроставитьКрайнийСрокУплаты(КоллекцияСтрокДвижений, Организация) Экспорт
	...
	НачалоУчетаОсобыхНерабочихДней = '21000101';
	ОкончаниеУчетаОсобыхНерабочихДней = '20200101';
	ПроизводственныйКалендарь = КалендарныеГрафики.ОсновнойПроизводственныйКалендарь();
	ОписаниеНерабочихДней = КалендарныеГрафики.ПериодыНерабочихДней(ПроизводственныйКалендарь, Новый СтандартныйПериод());
	Для каждого Описание Из ОписаниеНерабочихДней Цикл
		Для каждого День Из Описание.Даты Цикл
			НачалоУчетаОсобыхНерабочихДней = Мин(НачалоУчетаОсобыхНерабочихДней, День);
			ОкончаниеУчетаОсобыхНерабочихДней = Макс(ОкончаниеУчетаОсобыхНерабочихДней, День);
		КонецЦикла; 
	КонецЦикла; 
	ОкончаниеУчетаОсобыхНерабочихДней = КонецДня(ОкончаниеУчетаОсобыхНерабочихДней);
	...
	РабочиеДни = КалендарныеГрафики.БлижайшиеРабочиеДаты(ПроизводственныйКалендарь, ДатыУдержанияНалога, ПараметрыПолученияБлижайшихРабочихДат(Организация, УчитыватьОсобыеНерабочиеДни));
...
Функция ПараметрыПолученияБлижайшихРабочихДат(Организация, УчитыватьНерабочиеПериоды) 
	ПараметрыПолучения = КалендарныеГрафики.ПараметрыПолученияБлижайшихРабочихДат(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь());
	...
	Если УчитыватьНерабочиеПериоды Тогда
		...
		Если КоронавирусныеНерабочиеДни.ИгнорироватьДниС30МартаПо3Апреля Тогда
			ПараметрыПолучения.НерабочиеПериоды.Удалить(0)
		КонецЕсли;
		Если КоронавирусныеНерабочиеДни.ИгнорироватьДниС4По30Апреля Тогда
			ПараметрыПолучения.НерабочиеПериоды.Удалить(?(КоронавирусныеНерабочиеДни.ИгнорироватьДниС30МартаПо3Апреля,0,1))
		КонецЕсли;
		Если КоронавирусныеНерабочиеДни.ИгнорироватьДниС6По8Мая Тогда
			Если КоронавирусныеНерабочиеДни.ИгнорироватьДниС30МартаПо3Апреля И КоронавирусныеНерабочиеДни.ИгнорироватьДниС4По30Апреля Тогда
				ПараметрыПолучения.НерабочиеПериоды.Удалить(0)
			ИначеЕсли Не КоронавирусныеНерабочиеДни.ИгнорироватьДниС30МартаПо3Апреля И Не КоронавирусныеНерабочиеДни.ИгнорироватьДниС4По30Апреля Тогда
				ПараметрыПолучения.НерабочиеПериоды.Удалить(2)
			Иначе
				ПараметрыПолучения.НерабочиеПериоды.Удалить(1)
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция РегистрацииОрганизацииОтчетногоПериода(Организация, НачалоПериода, ОкончаниеПериода, ТолькоАктуальные = Ложь) Экспорт 
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ГоловнаяОрганизация", ЗарплатаКадры.ГоловнаяОрганизация(Организация));
	Запрос.УстановитьПараметр("МладшийПериод", ДобавитьМесяц(НачалоДня(НачалоПериода), -12));
	Запрос.УстановитьПараметр("СтаршийПериод", ДобавитьМесяц(КонецДня(ОкончаниеПериода), 3));
	Запрос.УстановитьПараметр("НачалоПериода", НачалоДня(НачалоПериода));
	...
	|	И СведенияОДоходах.Период МЕЖДУ &МладшийПериод И &СтаршийПериод
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ГоловнаяОрганизация = &ГоловнаяОрганизация
	|	И (РасчетыНалогоплательщиковСБюджетомПоНДФЛ.МесяцНалоговогоПериода МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|			ИЛИ РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Период < &ОкончаниеПериода)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.РегистрацияВНалоговомОргане <> ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Сумма * РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Сумма + РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СуммаСПревышения * РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СуммаСПревышения <> 0
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Период МЕЖДУ &МладшийПериод И &СтаршийПериод
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция СтрокаСведенийОДоходах(СведенияОДоходахНДФЛ, Организация, ДатаОперации, ДанныеДокумента, ОкончательныйРасчет = Истина, Ссылка = Неопределено, КатегорияДохода = Неопределено) Экспорт
	...
	Если КодыДоходовПоЦеннымБумагам(Год(НоваяСтрока.ДатаПолученияДохода)).Найти(НоваяСтрока.КодДохода) <> Неопределено Тогда
	...
	КонецЕсли;
	Если НоваяСтрока.ВключатьВДекларациюПоНалогуНаПрибыль И НоваяСтрока.ДатаПолученияДохода < ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли() Тогда
		НоваяСтрока.ВключатьВДекларациюПоНалогуНаПрибыль = Ложь
	КонецЕсли;
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(НоваяСтрока.СтрокаРаздела2Расчета6НДФЛ) Тогда
		НоваяСтрока.СтрокаРаздела2Расчета6НДФЛ = УчетНДФЛПовтИсп.СтрокаРаздела2Расчета6НДФЛПоНачислению(НоваяСтрока.Начисление)
	КонецЕсли;
...
Процедура СоздатьВТДанныеНДФЛПоДокументу(МенеджерВременныхТаблиц, ДокументСсылка, ОписаниеТаблицДокумента = Неопределено, ФизическиеЛица = Неопределено) Экспорт
	...
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ПримененныйВычет.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код323)
	|					ТОГДА ПримененныйВычет.РазмерВычета
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК ПримененныйВычетРасходыНаЛекарства,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	|				КОГДА ПримененныйВычет.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код329Спорт)
	|					ТОГДА ПримененныйВычет.РазмерВычета
	|				ИНАЧЕ 0
	|			КОНЕЦ, 0)) КАК ПримененныйВычетРасходыНаФизкультуруИСпорт,
	|	СУММА(ЕСТЬNULL(ВЫБОР
	...
	|	ЕСТЬNULL(ПримененныйВычет.ПримененныйВычетРасходыНаЛекарства, 0) КАК ПримененныйВычетРасходыНаЛекарства,
	|	ЕСТЬNULL(ПримененныйВычет.ПримененныйВычетРасходыНаФизкультуруИСпорт, 0) КАК ПримененныйВычетРасходыНаФизкультуруИСпорт,
...
//		ПримененныйВычетНаТретьегоРебенкаДвойнойВторойОпекунов,
//		ПримененныйВычетНаДетейИнвалидовОпекунов,ПримененныйВычетНаДетейИнвалидовДвойнойОпекунов,ПримененныйВычетНаДетейИнвалидовДвойнойВторойОпекунов
//		ПримененныйВычетРасходыНаСвоеОбучение,ПримененныйВычетРасходыНаОбучениеДетей,ПримененныйВычетРасходыНаЛечение,ПримененныйВычетСтраховыеВзносыНаМедУслуги,
//		ПримененныйВычетРасходыНаДорогостоящееЛечение,ПримененныйВычетВзносыНаДобровольноеСтрахованиеЖизни,ПримененныйВычетРасходыНаЛекарства,ПримененныйВычетРасходыНаФизкультуруИСпорт
//      	 
...
Функция ВычетыКДоходам(НалоговыйПериод) Экспорт
	...
	Если НалоговыйПериод = 2021 Или НалоговыйПериод = 2022 Тогда
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.КодДоходаПоУмолчанию, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код620));
	КонецЕсли;
	...
	СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код2720c2021, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код501));
...
Функция ВычетыНалогоплательщика(НалоговыйПериод, Знач ГруппаВычета = Неопределено) Экспорт
	...
	КонецЕсли;
	Если НалоговыйПериод > 2020 Тогда 
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код323);
	КонецЕсли;
	Если НалоговыйПериод > 2021 Тогда 
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код329Спорт);
	КонецЕсли;
...
Функция ВидыВычетовПоПолямТаблицы() Экспорт
	...
	ВидыВычетов.Вставить("ПримененныйВычетРасходыНаЛекарства", Справочники.ВидыВычетовНДФЛ.Код323);
	ВидыВычетов.Вставить("ПримененныйВычетРасходыНаФизкультуруИСпорт", Справочники.ВидыВычетовНДФЛ.Код329Спорт);
...
Процедура ПриПолученииСведенийОПоказателяхОтчета(ПоказателиОтчета, ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет6_НДФЛ" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв4" Тогда
			УчетНДФЛЗаполнениеОтчетности.ОписаниеПоказателей_6НДФЛ_2021Кв4(ПоказателиОтчета);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаполненииРегламентированногоОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет6_НДФЛ" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв4" Тогда
			УчетНДФЛЗаполнениеОтчетности.ЗначенияПоказателей_6НДФЛ_2021Кв4(ПараметрыОтчета, Контейнер);
			ПоместитьВоВременноеХранилище(Новый Структура("Контейнер", Контейнер), ПараметрыОтчета.АдресВоВременномХранилище);
		КонецЕсли;
	КонецЕсли;
...
Функция НоваяФормаРегламентированногоОтчета(ИДОтчета, ИДРедакцииОтчета) Экспорт
	...
	Возврат ИДОтчета = "РегламентированныйОтчет6_НДФЛ" И ИДРедакцииОтчета > "ФормаОтчета2021Кв4";	
...
Функция ВыводитьФорму2НДФЛ2022Года(НалоговыйПериод, ДатаДокумента) Экспорт

	ДатаНачалаДействия = '20220101';
	Возврат НалоговыйПериод >= Год(ДатаНачалаДействия)-1 И ДатаДокумента >= ДатаНачалаДействия
...
Процедура СправкиНДФЛПроверитьДанныеОДоходахНалогахВычетах(Ссылка, Дата, ДанныеСправки, ПутьКДаннымСотрудника, Доходы, Вычеты, СоответствиеСтавокДоходов, НачалоСообщения, Отказ, СводнаяСправка = Ложь, ПроверятьСоответствиеДоходовСтавке = Ложь, ДляНалогаНаПрибыль = Ложь, УведомленияНОоПравеНаВычеты = Неопределено, НомерКорректировки = 0, ПутьКДаннымДоходов = "", ПутьКДаннымВычетов = "", Ошибки = Неопределено) Экспорт
	...
	Иначе
		...
		ОтбиратьВычетыПоСтавке = Вычеты.Колонки.Найти("Ставка") <> Неопределено;
		Для каждого ЗначениеПеречисления Из Перечисления.НДФЛСтавки Цикл
			...
			Если ДоходыПоСтавке.Количество() > 0 
				...
				Если ОтбиратьВычетыПоСтавке Тогда
					ВсегоПрочихВычетов = Вычеты.Скопировать(Новый Структура("Ставка", ЗначениеПеречисления)).Итог("СуммаВычета");
				ИначеЕсли ЗначениеПеречисления = Перечисления.НДФЛСтавки.Ставка13 Тогда
					ВсегоПрочихВычетов = Вычеты.Итог("СуммаВычета") 
				Иначе
					ВсегоПрочихВычетов = 0 
				КонецЕсли;
				...
				Если ОблагаемаяСуммаДохода <> ДоходыПоСтавке.Итог("СуммаДохода") - ДоходыПоСтавке.Итог("СуммаВычета") - ВсегоПрочихВычетов Тогда
...
Функция СтавкаНДФЛ(СтавкаНалогообложенияРезидента, КодДохода, СтатусНалогоплательщика, НалоговыйПериод)	
...
Процедура ПрочитатьКлассификаторыВСоответствия(СоответствиеКодовСтран, СоответствиеИменСтран, СоответствиеДокументовУдостоверяющихЛичность, СоответствиеКодовДоходов, СоответствиеСтатусовНалогоплательщика, СоответствиеКодовДоходов2021 = Неопределено) Экспорт 
	...
	|	ДоходыНДФЛ.КодПрименяемыйВНалоговойОтчетностиС2021Года КАК КодДляОтчетностиС2021Года
	|ИЗ
	|	Справочник.ВидыДоходовНДФЛ КАК ДоходыНДФЛ";
	Выборка = Запрос.Выполнить().Выбрать();
	СоответствиеКодовДоходов = Новый Соответствие;
	Если СоответствиеКодовДоходов2021 <> Неопределено Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СоответствиеКодовДоходов2021 <> Неопределено Тогда
			СоответствиеКодовДоходов2021.Вставить(Выборка.Ссылка, Выборка.КодДляОтчетностиС2021Года)
		КонецЕсли;
	КонецЦикла;
...
//
// Возвращаемое значение:
//	В варианте ВРазрезеРегистраторов = Ложь. Структура со следующими полями:
...
//			ВключатьВДекларациюПоНалогуНаПрибыль
//
//	В варианте ВРазрезеРегистраторов = Истина. Структура со следующими полями:
//		Доходы - таблица значений с колонками:
//			ФизическоеЛицо,
//			Сотрудник,
//			Ставка,
...
//			ДатаПолученияДохода
//
Функция ДанныеОДоходахНалогахВычетах(Сотрудники, НалоговыйПериод, Организация, РегистрацияВНалоговомОргане = Неопределено, ДатаАктуальности = '00010101', КППОтправки = Неопределено, ВРазрезеРегистраторов = Ложь, ДополнительныйОтбор = "") Экспорт
	
	Возврат УчетНДФЛЗаполнениеОтчетности.ДанныеОДоходахНалогахВычетах(Сотрудники, НалоговыйПериод, Организация, РегистрацияВНалоговомОргане, ДатаАктуальности, КППОтправки, ВРазрезеРегистраторов, ДополнительныйОтбор)
...
Функция СформироватьПечатнуюФорму2НДФЛ(ОбъектыПечати, МассивОбъектов, ДанныеДокументов, ДанныеСотрудниковДокументов, ДанныеОДоходахДокументов, ДанныеОВычетахДокументов, ДанныеОбУведомленияхДокументов) Экспорт
	...
	Справка2022 = ПолучитьОбщийМакет("Форма2НДФЛс2022");
	...
	ОбластьМакетаШапкаПервойСтраницы_2022 = Справка2022.ПолучитьОбласть("ШапкаПервойСтраницы"); 
	ОбластьМакетаШапкаВторойСтраницы_2022 = Справка2021.ПолучитьОбласть("ШапкаВторойСтраницы");
	ОбластьМакетаЗаголовокРаздела3_2022 = Справка2022.ПолучитьОбласть("ЗаголовокРаздела3"); 
	ОбластьМакетаЗаголовокРаздела4_2022 = Справка2022.ПолучитьОбласть("ЗаголовокРаздела4");
	ОбластьМакетаРаздел5_2022 = Справка2022.ПолучитьОбласть("Раздел5");
	ОбластьМакетаРаздел6_2022 = Справка2022.ПолучитьОбласть("Раздел6");
	ОбластьМакетаНалоговыйАгент_2022 = Справка2022.ПолучитьОбласть("НалоговыйАгент");
	ОбластьМакетаПримечание_2022 = Справка2022.ПолучитьОбласть("Примечание");
	
	ОбластьМакетаСтрокаВычетов_2022 = Справка2022.ПолучитьОбласть("СтрокаВычетов"); 
	ОбластьМакетаСтрокаДоходов_2022 = Справка2022.ПолучитьОбласть("СтрокаДоходов"); 
	ОбластиМакетаДляДоходов_2022 = Новый Массив;
	ОбластиМакетаДляДоходов_2022.Добавить(Справка2022.ПолучитьОбласть("СтрокаДоходов|ЛеваяЧасть"));
	ОбластиМакетаДляДоходов_2022.Добавить(Справка2022.ПолучитьОбласть("СтрокаДоходов|ПраваяЧасть"));
	ОбластиМакетаДляВычетов_2022 = Новый Массив;
	Для Сч = 1 По 4 Цикл
		ОбластиМакетаДляВычетов_2022.Добавить(Справка2022.ПолучитьОбласть("СтрокаВычетов|Вычеты"+Сч));
	КонецЦикла;
	...
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		...
		Если ВыводитьФормуСправки2022 Тогда
			...
			ОбластьМакетаШапкаВторойСтраницы = ОбластьМакетаШапкаВторойСтраницы_2022;
			ОбластьМакетаЗаголовокРаздела3 = ОбластьМакетаЗаголовокРаздела3_2022; 
			ОбластьМакетаЗаголовокРаздела4 = ОбластьМакетаЗаголовокРаздела4_2022;
			ОбластьМакетаУведомлениеНаСоцВычет = Неопределено;
			ОбластьМакетаУведомлениеНаИмущВычет = Неопределено;
			ОбластьМакетаРаздел5 = ОбластьМакетаРаздел5_2022;
			ОбластьМакетаРаздел6 = ОбластьМакетаРаздел6_2022;
			ОбластьМакетаНалоговыйАгент = ОбластьМакетаНалоговыйАгент_2022;
			ОбластьМакетаПримечание = ОбластьМакетаПримечание_2022;
			
			ОбластьМакетаСтрокаВычетов = ОбластьМакетаСтрокаВычетов_2022; 
			ОбластьМакетаСтрокаДоходов = ОбластьМакетаСтрокаДоходов_2022; 
			ОбластиМакетаДляДоходов = ОбластиМакетаДляДоходов_2022;
			ОбластиМакетаДляВычетов = ОбластиМакетаДляВычетов_2022;
		ИначеЕсли ВыводитьФорму2НДФЛ2021Года(ДанныеТекущегоДокумента.НалоговыйПериод, ДанныеТекущегоДокумента.Дата) Тогда
		...
		Если ОбластьМакетаУведомлениеНаСоцВычет = Неопределено И ДанныеОВычетахДокументов.Колонки.Найти("Ставка") = Неопределено Тогда
			ДанныеОВычетахДокументов.Колонки.Добавить("Ставка", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
			ДанныеОВычетахДокументов.ЗаполнитьЗначения(Перечисления.НДФЛСтавки.Ставка13, "Ставка");
		КонецЕсли;
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Если ВыводитьФорму2НДФЛ2019Года(ДанныеТекущегоДокумента.НалоговыйПериод, ДанныеТекущегоДокумента.Дата) Тогда
				...
				Для Каждого СтрокаТЗ Из ДоходыПоВидам Цикл
					Если ПерваяСтрокаДохода = Неопределено 
						...
						СтрокаТЗ.КодДоходаСтрокой = СоответствиеДоходов[СтрокаТЗ.КодДохода];
			...
			Иначе
				Если ЗначениеЗаполнено(СтрокаДанных.Адрес) Тогда
					СтруктураАдреса = КонтактнаяИнформацияБЗК.СвернутьСведенияОбАдресе(КонтактнаяИнформацияБЗК.СведенияОбАдресеСТипами(СтрокаДанных.Адрес), 
				...
				Если ЗначениеЗаполнено(СтрокаДанных.АдресЗарубежом) Тогда
					СтруктураАдреса = КонтактнаяИнформацияБЗК.СведенияОбАдресеСТипами(СтрокаДанных.АдресЗарубежом);
					ОбластьМакетаШапкаПервойСтраницы.Параметры.КодСтр = СтруктураАдреса.КодСтраны;
			...
			Для Каждого СтавкаНДФЛ Из Перечисления.НДФЛСтавки Цикл
				...
				Если ОбластьМакетаУведомлениеНаСоцВычет = Неопределено Тогда
					...
					ВычетыПоСтавкам = ВычетыПоВидам.Скопировать(Новый Структура("Ставка",СтавкаНДФЛ));
					Если ВычетыПоСтавкам.Количество() > 0 Тогда
						ВременныйТабДокумент.Очистить();
						ВременныйТабДокумент.Вывести(ОбластьМакетаЗаголовокРаздела4);
						ВычетыПоСтавкам.Сортировать("КодВычета");
						ВывестиТабличныеДанные(ВычетыПоСтавкам, ВременныйТабДокумент, ОбластиМакетаДляВычетов, 1, 4);
						ВывестиРазделВСправку(ТабДокументТекущейСправки, ВременныйТабДокумент, Ложь, ОбластьМакетаНалоговыйАгент, ОбластьМакетаПримечание, ОбластьМакетаШапкаВторойСтраницы);
					ИначеЕсли ПерваяСтавка Тогда 
						ВременныйТабДокумент.Очистить();
						ВременныйТабДокумент.Вывести(ОбластьМакетаЗаголовокРаздела4);
				...
				Если Не ВыводитьФормуСправки2022 Тогда
				...
				Иначе
					ОбластьМакетаРаздел5.Параметры.НалогНаПрибыльДляДивидендов = УчетНДФЛКлиентСервер.СправкиНДФЛЗначениеИтоговПоСтавке(СтрокаДанных, "НалогНаПрибыльДляДивидендов", СтавкаНДФЛ);
				КонецЕсли;
				...
				Если ВыводитьФормуСправки2022 Тогда
					...
					ОбластьМакетаРаздел6.Параметры.Задолженность = УчетНДФЛКлиентСервер.СправкиНДФЛЗначениеИтоговПоСтавке(СтрокаДанных, "Задолженность", СтавкаНДФЛ); 
					ОбластьМакетаРаздел6.Параметры.СуммаДоходаНеудержанного = УчетНДФЛКлиентСервер.СправкиНДФЛЗначениеИтоговПоСтавке(СтрокаДанных, "СуммаДоходаНеудержанного", СтавкаНДФЛ);
				
					ВременныйТабДокумент.Вывести(ОбластьМакетаРаздел6);
					ВывестиРазделВСправку(ТабДокументТекущейСправки, ВременныйТабДокумент, ВыведеноИтогов = ВсегоИтогов, ОбластьМакетаНалоговыйАгент, ОбластьМакетаПримечание, ОбластьМакетаШапкаВторойСтраницы);
				КонецЕсли;
...
Функция ОписаниеДоходовОрганизации(Организация, НалоговыйПериод, ДатаПодачиСведений, ФизическоеЛицо = Неопределено, ДляНалогаНаПрибыль = Ложь, НеВключатьДоходыПоЦеннымБумагам = Ложь) Экспорт
	...
	|	И НЕ НДФЛСведенияОДоходах.КодДохода.НеОблагаетсяУНалоговогоАгента
	|	И &УсловиеКодДохода
...
Процедура СоздатьВТРегистрацииВНалоговомОрганеОрганизацииОКАТОКПП(Организация, МенеджерВременныхТаблиц) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО ИсторияРегистрацийВНалоговомОргане.СтруктурнаяЕдиница = Организации.Ссылка
	|ГДЕ
...
Функция РезультатПроверкиКорректностиКодовДохода()

	Результат = Новый Структура("Документы, Справочники");
	...
	|ИЗ
	|	РегистрНакопления.СведенияОДоходахНДФЛ КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыДоходовНДФЛ КАК ВидыДоходовНДФЛ
	|		ПО Таблица.КодДохода = ВидыДоходовНДФЛ.Ссылка
	|ГДЕ
	|	Таблица.КодДохода <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ПустаяСсылка)
	|	И Таблица.КодДохода ССЫЛКА Справочник.ВидыДоходовНДФЛ
	|	И ВидыДоходовНДФЛ.Ссылка ЕСТЬ NULL";
	Результат.Вставить("Документы", Запрос.Выполнить());
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыДоходовНДФЛ КАК ВидыДоходовНДФЛ
	|		ПО Начисления.КодДоходаНДФЛ = ВидыДоходовНДФЛ.Ссылка
	|ГДЕ
	|	Начисления.КодДоходаНДФЛ <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ПустаяСсылка)
	|	И Начисления.КодДоходаНДФЛ ССЫЛКА Справочник.ВидыДоходовНДФЛ
	|	И ВидыДоходовНДФЛ.Ссылка ЕСТЬ NULL";
	ЗапросПриемник = СтрЗаменить(ШаблонЗапроса, "&Начисления", "ПланВидовРасчета.Начисления");
	Для каждого ОписаниеСправочника Из Метаданные.Справочники Цикл
		Если ОписаниеСправочника.Реквизиты.Найти("КодДоходаНДФЛ") <> Неопределено Тогда
			ЗапросПриемник = ЗапросПриемник + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + СтрЗаменить(ШаблонЗапроса, "&Начисления", ОписаниеСправочника.ПолноеИмя())
		КонецЕсли;
	КонецЦикла;
	
	Запрос.Текст = ЗапросПриемник;
	Результат.Вставить("Справочники", Запрос.Выполнить());
	
	Возврат Результат
	
КонецФункции

Функция РезультатПроверкиКорректностиКодовВычетов()
	...
	|	РегистрНакопления.СведенияОДоходахНДФЛ КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыВычетовНДФЛ КАК ВидыВычетовНДФЛ
	|		ПО Таблица.КодВычета = ВидыВычетовНДФЛ.Ссылка
	|ГДЕ
	|	Таблица.КодВычета <> ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
	|	И Таблица.КодВычета ССЫЛКА Справочник.ВидыВычетовНДФЛ
	|	И ВидыВычетовНДФЛ.Ссылка ЕСТЬ NULL
	|	И Таблица.Период >= &НачалоОсмотра
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.Регистратор
	|ИЗ
	|	РегистрНакопления.ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыВычетовНДФЛ КАК ВидыВычетовНДФЛ
	|		ПО Таблица.КодВычета = ВидыВычетовНДФЛ.Ссылка
	|ГДЕ
	|	Таблица.КодВычета <> ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
	...
	|	Таблица.Регистратор
	|ИЗ
	|	РегистрНакопления.ИмущественныеВычетыНДФЛ КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыВычетовНДФЛ КАК ВидыВычетовНДФЛ
	|		ПО Таблица.КодВычета = ВидыВычетовНДФЛ.Ссылка
	|ГДЕ
	|	Таблица.КодВычета <> ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
	...
	|	И Таблица.КодВычета ССЫЛКА Справочник.ВидыВычетовНДФЛ
	|	И ВидыВычетовНДФЛ.Ссылка ЕСТЬ NULL";
	Запрос.УстановитьПараметр("НачалоОсмотра", ДобавитьМесяц(НачалоГода(ТекущаяДатаСеанса()), -12));
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ВидыДоходовНДФЛ) Тогда
		...
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.17.119";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ЗаполнитьКодыДоходов2021";
		Обработчик.ОбщиеДанные = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.18.70";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ВосстановитьКоды1400и2400";
		Обработчик.ОбщиеДанные = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.20.67";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ПереименоватьКодыДоходовДекабрь2021";
		Обработчик.ОбщиеДанные = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.20.90";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ОписатьКодыПодарков";
		Обработчик.ОбщиеДанные = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.21.14";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ПроставитьКод2021Года";
		Обработчик.ОбщиеДанные = Истина;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.21.4";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ОписатьДоходы3020и3023";
		Обработчик.ОбщиеДанные = Истина;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ВидыВычетовНДФЛ) Тогда
		...
		Обработчик.Процедура = "Справочники.ВидыВычетовНДФЛ.ОписатьВычеты2022";
		Обработчик.ОбщиеДанные = Истина;
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.17.167";
	Обработчик.Процедура = "УчетНДФЛ.УточнитьКрайнийСрокУплатыВМае2021";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5e4a91fe-3bb4-49d5-8a6f-91757e42ed18");
	Обработчик.Комментарий = НСтр("ru = 'Исправление сроков уплаты в мае 2021'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.20.6";
	Обработчик.Процедура = "УчетНДФЛ.УточнитьКрайнийСрокУплатыВНоябре2021";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3484c7ae-b514-4499-8d5b-28a5511a898e");
	Обработчик.Комментарий = НСтр("ru = 'Исправление сроков уплаты в ноябре 2021'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.21.30";
	Обработчик.Процедура = "Документы.СправкаНДФЛ.ПроставитьСтавкуВТаблицеВычетов";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("403b42c2-98dc-41e7-acdc-a7c80bc7e115");
	Обработчик.Комментарий = НСтр("ru = 'Обработка вычетов в справках о доходах для сотрудников.'");
...
Процедура УточнитьКрайнийСрокУплатыВМае2021(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВариантУдержания = ЗНАЧЕНИЕ(Перечисление.ВариантыУдержанияНДФЛ.Удержано)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.КрайнийСрокУплаты МЕЖДУ ДАТАВРЕМЯ(2021, 5, 4, 0, 0, 0) И ДАТАВРЕМЯ(2021, 5, 7, 0, 0, 0)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Период >= ДАТАВРЕМЯ(2021, 4, 1, 0, 0, 0)";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПересмотретьСрокиУплаты(Выборка.Организация, '20210504', '20210507');
	КонецЦикла;

КонецПроцедуры

Процедура УточнитьКрайнийСрокУплатыВНоябре2021(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВариантУдержания = ЗНАЧЕНИЕ(Перечисление.ВариантыУдержанияНДФЛ.Удержано)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.КрайнийСрокУплаты МЕЖДУ ДАТАВРЕМЯ(2021, 10, 30, 0, 0, 0) И ДАТАВРЕМЯ(2021, 11, 7, 0, 0, 0)
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Период МЕЖДУ ДАТАВРЕМЯ(2021, 9, 1, 0, 0, 0) И ДАТАВРЕМЯ(2021, 11, 10, 0, 0, 0)";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПересмотретьСрокиУплаты(Выборка.Организация, '20211030', '20211107');
	КонецЦикла;
