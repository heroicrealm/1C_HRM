...
Функция ПараметрыОтправитьСообщениеСЭДО(
		...
		ОписаниеОшибки = "",
		РегистрационныйНомерФСС = "") Экспорт
	...
	Если ЗначениеЗаполнено(Результат.Организация) И НЕ ЗначениеЗаполнено(Результат.РегистрационныйНомерФСС) Тогда
		УстановитьПривилегированныйРежим(Истина);
		КонтекстЭДО = ДокументооборотСКО.ПолучитьОбработкуЭДО();
		СвойстваОрганизацииДляОбмена = КонтекстЭДО.СвойстваОрганизацииДляОбмена(Результат.Организация);
		УстановитьПривилегированныйРежим(Ложь);
		Результат.РегистрационныйНомерФСС = ?(ЗначениеЗаполнено(СвойстваОрганизацииДляОбмена.РегистрационныйНомерФСС),
			СокрЛП(СвойстваОрганизацииДляОбмена.РегистрационныйНомерФСС), Неопределено);
	КонецЕсли;
