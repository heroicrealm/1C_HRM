...
// Выводит сумму по буквам в секцию табличного документа.
//
// Параметры:
//   Секция  - Структура - См. УправлениеПечатьюБЗК.СекцияПечатнойФормы;
...
//
Процедура ВывестиСуммуРублейКопеекПоБуквам(Секция, Сумма, Префикс) Экспорт
	ВыравниваниеПоЛевомуКраю = Ложь;
	СуммаРублей = Формат(Цел(Сумма), "ЧДЦ=0; ЧН=; ЧГ=");
	СуммаКопеек = Формат((Сумма - Цел(Сумма))*100, "ЧЦ=2; ЧДЦ=0; ЧН=; ЧВН=; ЧГ=");
	ВывестиПоБуквам(Секция, СуммаРублей, Префикс + "_Рублей", ВыравниваниеПоЛевомуКраю);
	ВывестиПоБуквам(Секция, СуммаКопеек, Префикс + "_Копеек", ВыравниваниеПоЛевомуКраю);
КонецПроцедуры
