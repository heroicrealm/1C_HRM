...
Функция ОбменССерверомСоздатьСервис(
	...
	Возврат ДокументооборотСКОПовтИсп.ОбменССерверомСоздатьСервис(
		СпецОператорСвязи, 
		ЗапомнитьОшибку, 
		ОписаниеОшибки, 
		ПараметрыРесурса, 
...
Функция ПолучитьГруппуНовогоСобытияДокументооборотаСКО(Запись)
	...
	НезавершенныеСостояния = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		"Отправлено оператору;Отправлено в ФНС;Отправлено в ПФР;Отправлено в ФСС;Отправлено в Росстат;Отправлено в ФСРАР;"
		+ "Отправлено в РПН;Отправлено в ФТС;Принято в обработку;Отправлено в Банк России", ";");
