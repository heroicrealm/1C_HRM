...
Процедура ОбновитьНадписьВБольничном(Надпись, Больничный, ПособиеВыплачиваетсяФСС) Экспорт
...
Процедура ВывестиШапкуИПодвалЗаявленияОВозмещении_2012(Макет, Выборка) Экспорт
	...
	Если ЗначениеЗаполнено(Выборка.АдресОрганизации) Тогда
		СтруктураАдреса = СтруктураАдресаВМашиночитаемомФормате2012(Выборка.АдресОрганизации);
...
Функция КодыПричинИсправления(КодыПричин = Неопределено) Экспорт
	Возврат СЭДОФСС.СписокВыбораПричинИсправления(КодыПричин);
КонецФункции
...
Функция СоздатьВТЗаявления(Запрос, ТаблицаОснований, ВидРеестра) Экспорт
	...
	Иначе
		...
		Если ВидРеестра = Перечисления.ВидыРеестровСведенийНеобходимыхДляНазначенияИВыплатыПособий.ЕжемесячныеПособияПоУходуЗаРебенком Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеестрСведенийНеобходимыхДляНазначенияИВыплатыПособий.СведенияНеобходимыеДляНазначенияПособий КАК ТаблицаСведений
			|		ПО ПрекращаемыеЗаявления.Заявление = ТаблицаСведений.Заявление
			|			И ПрекращаемыеЗаявления.Ссылка = ТаблицаСведений.ПервичныйДокумент
			|			И (ТаблицаСведений.Ссылка <> &СсылкаРеестра)
			|			И (ТаблицаСведений.Ссылка.ПометкаУдаления = ЛОЖЬ)
			|ГДЕ
