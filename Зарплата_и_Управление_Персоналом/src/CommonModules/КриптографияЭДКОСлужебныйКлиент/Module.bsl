...
Процедура РасшифроватьPKCS7СервисомОблачнойПодписиРезультат(РезультатВызова, ВходящийКонтекст) Экспорт
	...
	ТекстОшибки = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(РезультатВызова, "Ошибка", "");
	Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ТекстОшибки = СервисКриптографииDSSКлиентСервер.ПолучитьПолеСтруктуры(РезультатВызова, "ОписаниеОшибки", "");
	КонецЕсли;	
...
Функция ИспользованиеОблачнойПодписиВозможно() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяОблачнаяПодпись")
...
Функция ПолучитьМодульПодсистемы(ИмяМодуля, ИмяПодсистемы = "")
	...
	Иначе
		ЕстьПодсистема = ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяОблачнаяПодпись")
