...
//                          - Массив, массив ссылок на регистраторы, данные которых необходиом исключить
//
// Возвращаемое значение:
//  Булево
//
...
Функция ПоследнееЗаявлениеОВеденииТрудовойКнижки(ФизическоеЛицо, ДатаСведений, ИсключаемыеРегистраторы) Экспорт
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
		"ВТОтборФизическогоЛица", "ФизическоеЛицо");
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаявленияОВеденииТрудовыхКнижек.ПериодЗаписи КАК Дата,
		|	ЗаявленияОВеденииТрудовыхКнижек.ВидЗаявления КАК ВидЗаявления
		|ИЗ
		|	ВТЗаявленияОВеденииТрудовыхКнижекСрезПоследних КАК ЗаявленияОВеденииТрудовыхКнижек";
...
// Устарела. Следует использовать механизм подсистемы БлокировкаИзмененияОбъектов.
...
Функция ДанныеТрудовойДеятельностиСотрудников(ТолькоРазрешенные, Организация, ФизическоеЛицо = Неопределено, НачалоПериода = Неопределено, ОкончаниеПериода = Неопределено, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено, ДляПоДневногоОтчета = Ложь) Экспорт
	...
	Возврат ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(Запрос.МенеджерВременныхТаблиц, ТолькоРазрешенные, ОрганизацииСбораДанных, МероприятияВсехСотрудников, ИсключаемыйРегистратор, ДляПоДневногоОтчета);
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	Настройка.ИдентификаторПечатнойФормы = Метаданные.Документы.ЗаявленияОПредоставленииСведенийОТрудовойДеятельности.Макеты.ПФ_MXL_ЗаявленияОПредоставленииСведенийОТрудовойДеятельности.Имя;
	Настройка.СодержимоеДокумента = Перечисления.СодержимоеДокументов.НеСодержитЗарплаты;
	
	Настройка = ОписанияНастроек.Добавить();
	Настройка.ИдентификаторПечатнойФормы = Метаданные.Документы.СведенияОТрудовойДеятельностиРаботникаСТД_Р.Макеты.ПФ_MXL_СТД_Р.Имя;
	Настройка.СодержимоеДокумента = Перечисления.СодержимоеДокументов.НеСодержитЗарплаты;
...
Функция ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(МенеджерВременныхТаблиц, ТолькоРазрешенные, Организации, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено, ДляПоДневногоОтчета = Ложь)
...
Функция ЗапросВТЗаявленияСотрудников(ТолькоРазрешенные, ПараметрыПостроения, ИмяВТЗаявленияСотрудников = "ВТЗаявленияСотрудников", ИсключаемыйРегистратор = Неопределено, ПоляОтбораПериодическихДанных = Неопределено)
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
		ПараметрыПостроения.ИмяВТСотрудникиПериоды, "ФизическоеЛицо");
	...
	ПараметрыПостроенияСреза.СоответствиеПсевдонимовПолейСКД.Вставить("Организация", "ОрганизацияЗаявления");
	...
	ТекстЗапросаВТЗаявленияСотрудников =
		...
		|	ЗаявленияОВеденииТрудовыхКнижекСрезПоследних.Организация,
