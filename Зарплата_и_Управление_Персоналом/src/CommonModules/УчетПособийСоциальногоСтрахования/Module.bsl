...
//   См. части 2 и 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
//
// Параметры:
//   КодПричины                   - Строка(2)
...
Функция ДнейОплатыЗаСлучай(КодПричины, ВозрастРодственникаПолныхЛет) Экспорт
	Если КодПричины = "08" Тогда
		Возврат 24; // Часть 2 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли КодПричины = "09" И ВозрастРодственникаПолныхЛет >= 16 Тогда
		Возврат 7; // Пункт 6 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли КодПричины = "09" И ВозрастРодственникаПолныхЛет >= 7 Тогда
		Возврат 15; // Пункт 2 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	КонецЕсли;
	Возврат Неопределено;
КонецФункции

// Возвращает максимальное количество оплачиваемых дней нетрудоспособности по уходу за родственником в год.
//   См. часть 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
//
// Параметры:
...
Функция ДнейУходаЗаРодственникомВГод(СлучайУходаЗаБольнымРебенком) Экспорт
	Если СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетАмбулаторно
		...
		Возврат 60; // Пункт 1 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетПриОсобомЗаболеванииАмбулаторно
		...
		Возврат 90; // Пункт 1 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До7ПолныхЛетАмбулаторно
		...
		Возврат 45; // Пункт 2 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ИнвалидомДо8ЛетАмбулаторно
		...
		Возврат 120; // Пункт 3 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До8ЛетПриПоствакцинальномОсложненииАмбулаторно
		...
		Возврат Неопределено; // Пункты 4 и 5 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	Иначе
		Возврат 30; // Пункт 6 части 5 статьи 6 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	КонецЕсли;
КонецФункции

// Возвращает количество оплачиваемых дней по уходу за ребенком перед началом половинной оплаты.
//   См. часть 3 статьи 7 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
//
// Параметры:
...
Функция ДнейПолнойОплатыПередПоловинной(СлучайУходаЗаБольнымРебенком, ДатаНачалаСобытия) Экспорт
	...
	Если СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетАмбулаторно
		...
		Возврат 10; // Пункт 1 части 3 статьи 7 Федерального закона от 29 декабря 2006 г. № 255-ФЗ.
	Иначе
		Возврат Неопределено;
	КонецЕсли;
КонецФункции

// См. часть 3 статьи 7 Федерального закона от 29 декабря 2006 г. № 255-ФЗ в редакции от 29.12.2020.
...
Функция ПричинаНетрудоспособности(КодПричины, ВозрастРодственника) Экспорт
	...
	ИначеЕсли КодПричины = "04" Или КодПричины = "07" Тогда
...
Процедура ЗаполнитьСписокВыбораСлучаевУходаЗаБольнымРебенком(Больничный, ПолеВвода, ТаблицаСлучаевУхода) Экспорт
	...
	Иначе
		КодыПричины = Перечисления.ПричиныНетрудоспособности.КодыПричины(Больничный.ПричинаНетрудоспособности);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Случай) И СписокВыбора.НайтиПоЗначению(Случай) = Неопределено Тогда
		СтрокаТаблицы = ТаблицаСлучаевУхода.Найти(Случай, "Значение");
