...
Функция ПолучитьСертификаты(Знач Хранилище) Экспорт
	...
	Для Каждого ЗначениеХранилища Из Хранилище Цикл
		...
		ДополнитьСертификатыСервера(Сертификаты, МассивСертификатов, ЗначениеХранилища, МестоХраненияКлюча);
...
Функция ПолучитьСвойстваСертификата(Знач ДанныеСертификата, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // Certificate ::= SEQUENCE...
			Возврат СвойстваСертификата;
		КонецЕсли;
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // TBSCertificate ::= SEQUENCE...
			Возврат СвойстваСертификата;
		КонецЕсли;
		...
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // signature AlgorithmIdentifier = SEQUENCE {...}
			Возврат СвойстваСертификата;
		КонецЕсли;
		ПропуститьБлок(ОбъектЧтениеДанных, СвойстваБлока.РазмерБлока);
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // issuer RDNSequence ::= SEQUENCE {...}
			Возврат СвойстваСертификата;
		КонецЕсли;
		ПропуститьБлок(ОбъектЧтениеДанных, СвойстваБлока.РазмерБлока);
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // validity Validity ::= SEQUENCE {...}
			Возврат СвойстваСертификата;
		КонецЕсли;
		ПропуститьБлок(ОбъектЧтениеДанных, СвойстваБлока.РазмерБлока);
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // subject RDNSequence ::= SEQUENCE {...}
			Возврат СвойстваСертификата;
		КонецЕсли;
		ПропуститьБлок(ОбъектЧтениеДанных, СвойстваБлока.РазмерБлока);
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // subjectPublicKeyInfo SubjectPublicKeyInfo ::= SEQUENCE...
			Возврат СвойстваСертификата;
		КонецЕсли;
		КонецБлока = СвойстваБлока.РазмерБлока + ОбъектЧтениеДанных.ИсходныйПоток().ТекущаяПозиция();
		
		СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
		Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда // algorithm AlgorithmIdentifier = SEQUENCE...
			Возврат СвойстваСертификата;
		КонецЕсли;
		...
		Если СвойстваБлока.Класс = "CONTEXT-SPECIFIC" И СвойстваБлока.Тег = Типы.BIT_STRING Тогда
			...
			СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
			Если НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных) Тогда
				Возврат СвойстваСертификата;
			КонецЕсли;
		...
		КонецЕсли;
	Исключение
		ИмяСобытия = ?(КриптографияЭДКОКлиентСервер.ЭтоПодписьСервиса(НастройкиВызова.МестоХраненияКлюч),
...
Функция ПрочитатьOID2_5_29_35(ОбъектЧтениеДанных, СвойстваСертификата)
	...
	СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
	Если СвойстваБлока.Класс = "UNIVERSAL" И СвойстваБлока.Тег = Типы().IA5String Тогда
...
Функция НужноПрерватьЧтениеДляIA5String(СвойстваБлока, ОбъектЧтениеДанных)
...
Функция Типы()
	...
	Типы.Вставить("IA5String", 16);  // Произвольная строка символов IA5 (ASCII)
