...
Процедура СоздатьВТПерсональныеДанныеСотрудников(МенеджерВременныхТаблиц, Организация, Год, ДатаДокумента, ФизическиеЛица = Неопределено) Экспорт 
	...
	КадровыеДанные = "ФизическоеЛицо, Фамилия, Имя, Отчество, СтраховойНомерПФР, ДатаУвольнения, ЯвляетсяПрокурором, ЯвляетсяВоеннослужащим, РаботаетВСтуденческомОтряде, ЯвляетсяОпекуномПенсионером";
...
Функция ЗапросПоСотрудникамДляПечати(МассивСсылок, СписокСотрудников = Неопределено) Экспорт
