...
Функция ИсключаемыеРегистраторы()
	...
	Если ИсправлениеДокументовЗарплатаКадрыКлиентСервер.ЭтоИсправление(ЭтотОбъект) Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, 
			ИсправлениеДокументовЗарплатаКадры.ПолучитьДокументыЦепочкиИсправлений(Объект.Ссылка));
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьПериодПриостановки(СсылкаНаБольничный, Идентификатор)
	...
	Если Не (ОбщегоНазначенияБЗК.ДатаВИнтервале(ДанныеБольничногоЛиста.ДатаНачала, 
		ПериодОтсутствия.НачалоПериода, ПериодОтсутствия.ОкончаниеПериода) 
	...
	ПериодПересечения = ОбщегоНазначенияБЗК.ПериодПересеченияДвухИнтервалов(ДанныеБольничногоЛиста.ДатаНачала, 
		ДанныеБольничногоЛиста.ДатаОкончания,ПериодОтсутствия.НачалоПериода, ПериодОтсутствия.ОкончаниеПериода);
