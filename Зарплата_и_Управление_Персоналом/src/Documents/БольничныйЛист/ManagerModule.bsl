...
// Устарела. Следует использовать УчетПособийСоциальногоСтрахования.ДнейОплатыЗаСлучайНетрудоспособности.
...
Процедура ЗаполнитьОтветНаЗапросФССДляРасчетаПособия(Больничный, ДокументСЭДО, Реквизиты, ДанныеДляРасчетаСреднегоЗаработка) Экспорт
	...
	РазностьСтажей = УчетПособийСоциальногоСтрахования.ПодсчитатьРазностьСтажейВГодахИМесяцах(
		РеквизитыБольничного.СтажРасширенныйЛет,
		РеквизитыБольничного.СтажРасширенныйМесяцев,
		РеквизитыБольничного.СтажЛет,
		РеквизитыБольничного.СтажМесяцев);
	Если РазностьСтажей.РазностьЛет <> 0 Или РазностьСтажей.РазностьМесяцев <> 0 Тогда
		...
		Реквизиты.Вставить("НестраховойСтажЛет",     РазностьСтажей.РазностьЛет);
		Реквизиты.Вставить("НестраховойСтажМесяцев", РазностьСтажей.РазностьМесяцев);
	Иначе
...
Процедура ПриЗаполненииСсылокРодственников(Больничный, РодственникиЭЛН) Экспорт
	РодственникиТребующиеЗаполнения = Новый Массив;
	Для Каждого РодственникЭЛН Из РодственникиЭЛН Цикл
		Если Не ЗначениеЗаполнено(РодственникЭЛН.Ссылка) Тогда
			РодственникиТребующиеЗаполнения.Добавить(РодственникЭЛН);
		КонецЕсли;
	КонецЦикла;
	Если РодственникиТребующиеЗаполнения.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Владелец", Больничный.ФизическоеЛицо);
	
	БыстрыйПоискРодственников = Новый Соответствие;
	ТаблицаСправочника = Запрос.Выполнить().Выгрузить();
	Для Каждого РодственникИБ Из ТаблицаСправочника Цикл
		ДополнитьБыстрыйПоискРодственников(РодственникИБ, БыстрыйПоискРодственников);
	КонецЦикла;
	
	Для Каждого РодственникЭЛН Из РодственникиТребующиеЗаполнения Цикл
		Для Каждого Ключ Из РодственникЭЛН.Ключи Цикл
			РодственникИБ = БыстрыйПоискРодственников[Ключ];
			Если РодственникИБ = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если ЗначениеЗаполнено(РодственникЭЛН.СНИЛС)
				И ЗначениеЗаполнено(РодственникИБ.СНИЛС)
				И РодственникЭЛН.СНИЛС <> РодственникИБ.СНИЛС Тогда
				Продолжить;
			КонецЕсли;
			РодственникЭЛН.Ссылка = РодственникИБ.Ссылка;
			ОбновитьРеквизитыРодственникаИБ(Больничный, РодственникЭЛН, РодственникИБ, БыстрыйПоискРодственников);
			Прервать;
		КонецЦикла;
		Если Не ЗначениеЗаполнено(РодственникЭЛН.Ссылка)
			И ЗначениеЗаполнено(РодственникЭЛН.СНИЛС)
			И ЗначениеЗаполнено(РодственникЭЛН.Фамилия)
			И ЗначениеЗаполнено(РодственникЭЛН.Имя)
			И ЗначениеЗаполнено(РодственникЭЛН.Отчество)
			И ЗначениеЗаполнено(РодственникЭЛН.ДатаРождения) Тогда
			РодственникЭЛН.Ссылка = СоздатьРодственникаИБ(Больничный, РодственникЭЛН, БыстрыйПоискРодственников);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуУходаЗаРодственниками(Больничный, ДанныеЭЛН) Экспорт
	...
	ОбменЛисткамиНетрудоспособностиФСС.ЗаполнитьДатыУходаЗаРодственниками(Больничный, ДанныеЭЛН);
...
Функция ОбъектЗаблокирован(СсылкаНаОбъект) Экспорт
	...
	Если Не ПравоДоступа("Добавление", Метаданные.Документы.БольничныйЛист) Тогда
...
Процедура ПриПечатиДоплатыЗаДниНетрудоспособности(КонтекстПечати)
	...
	Пока ДанныеЛН.Следующий() Цикл
		...
		СекцияПриказ.Параметры.ФамилияИО = ФамилияИнициалыВПадеже(КадровыеДанныеСотрудника, 2);
...
// Склоняет переданные фамилию, имя, отчество и возвращает "Фамилия И. О." в указанном падеже.
//
// Параметры:
//   КадровыеДанныеСотрудника - СтрокаТаблицыЗначений, Структура
//       * ФизическоеЛицо
//       * Пол
//       * ФИОПолные
...
//   Строка
//
Функция ФамилияИнициалыВПадеже(КадровыеДанныеСотрудника, Падеж)
	ФИОПолные = КадровыеДанныеСотрудника.ФИОПолные;
	ФизическиеЛицаЗарплатаКадры.Просклонять(ФИОПолные, Падеж, ФИОПолные, КадровыеДанныеСотрудника.Пол, КадровыеДанныеСотрудника.ФизическоеЛицо);
	Возврат ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ФИОПолные);
КонецФункции
...
Функция РеквизитыДокументаДляРасчетаСреднегоЗаработкаФСС(БольничныйОбъект)
	...
	РеквизитыДокумента.ИспользоватьДниБолезниУходаЗаДетьми = 
		БольничныйОбъект.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам
		И БольничныйОбъект.ПериодРегистрации >= УчетПособийСоциальногоСтрахованияКлиентСервер.ДатаОтдельногоРасчетаПособийПоМатеринству();
...
Процедура ВставитьСсылкиРодственниковВДанныеЭЛН(Больничный, ДанныеЭЛН)
	// Получение периодов лечения из ЭЛН.
	ПериодыЛечения = ОбменЛисткамиНетрудоспособностиФСС.ПериодыЛеченияРодственников(ДанныеЭЛН);
	
	// Заполнение соответствия для быстрого поиска периодов лечения по родственнику.
	ПоискПериодовЛечения = Новый Соответствие;
	Для Каждого ПериодЛечения Из ПериодыЛечения Цикл
...
Функция СоздатьРодственникаИБ(Больничный, РодственникЭЛН, БыстрыйПоискРодственников)
	РодственникОбъект = Справочники.РодственникиФизическихЛиц.СоздатьЭлемент();
	РодственникОбъект.Владелец     = Больничный.ФизическоеЛицо;
	РодственникОбъект.Фамилия      = РодственникЭЛН.Фамилия;
	РодственникОбъект.Имя          = РодственникЭЛН.Имя;
	РодственникОбъект.Отчество     = РодственникЭЛН.Отчество;
	РодственникОбъект.Наименование = РодственникЭЛН.ФИО;
	РодственникОбъект.ДатаРождения = РодственникЭЛН.ДатаРождения;
	РодственникОбъект.СНИЛС        = РодственникЭЛН.СНИЛС;
	РодственникОбъект.КодСвязи     = РодственникЭЛН.КодСвязи;
...
Процедура ОбновитьРеквизитыРодственникаИБ(Больничный, РодственникЭЛН, РодственникИБ, БыстрыйПоискРодственников)
...
Функция ЗаполнитьСлучайУходаЗаБольнымРебенком(Больничный, ДатыУходаЗаРодственниками) Экспорт
	...
	Если ЕстьИзменения Тогда
		Причина = УчетПособийСоциальногоСтрахования.ПричинаНетрудоспособности(КодПричины, ВозрастЛет);
		Больничный.ПричинаНетрудоспособности = Причина;
	КонецЕсли;
...
Процедура ДобавитьДатыКИсключениюПоПричинеОтпуска(Больничный, ДатыКИсключению)
	...
	Запрос.УстановитьПараметр("ПричинаИное",
		Перечисления.ПричиныНеоплатыБольничных.Иное);
	Запрос.УстановитьПараметр("ПричинаОтпуск",
		Перечисления.ПричиныНеоплатыБольничных.Отпуск);
	Запрос.УстановитьПараметр("ПричинаУчебныйОтпуск",
		Перечисления.ПричиныНеоплатыБольничных.УчебныйОтпуск);
	Запрос.УстановитьПараметр("ПричинаОтпускБезСохраненияОплаты",
		Перечисления.ПричиныНеоплатыБольничных.ОтпускБезСохраненияОплаты);
	Запрос.УстановитьПараметр("ОтпускОсновной",
		Перечисления.СостоянияСотрудника.ОтпускОсновной);
	Запрос.УстановитьПараметр("ДополнительныйОтпуск",
		Перечисления.СостоянияСотрудника.ДополнительныйОтпуск);
	Запрос.УстановитьПараметр("ОтпускУчебныйОплачиваемый",
		Перечисления.СостоянияСотрудника.ОтпускУчебныйОплачиваемый);
	Запрос.УстановитьПараметр("ОтпускУчебныйНеоплачиваемый",
		Перечисления.СостоянияСотрудника.ОтпускУчебныйНеоплачиваемый);
	Запрос.УстановитьПараметр("ОтпускНеоплачиваемыйПоЗаконодательству",
		Перечисления.СостоянияСотрудника.ОтпускНеоплачиваемыйПоЗаконодательству);
	Запрос.УстановитьПараметр("ОтпускНеоплачиваемыйПоРазрешениюРаботодателя",
		Перечисления.СостоянияСотрудника.ОтпускНеоплачиваемыйПоРазрешениюРаботодателя);
	...
	Для Каждого Дата Из МассивДат Цикл
		...
		Если ЕстьДатыУхода Тогда
			...
			Если ДатаУхода = Неопределено Тогда
				...
				КонецЕсли;
			Иначе
				// Оплачиваемость даты определяется параметрами на странице "Оплата".
	...
	Для Каждого ПериодУхода Из Больничный.ПериодыУходаЗаРодственниками Цикл
		...
		Если РассчитатьПериодыУхода И Не ПериодУхода.КоличествоДнейИзменено Тогда
			...
			Иначе
				...
				Иначе
					ОстатокДней = Мин(СтрокаОстатковЗаГод.ОстатокДнейЗаГод, СтрокаОстатковЗаСлучай.ОстатокДнейЗаСлучай);
				КонецЕсли;
		...
		КонецЕсли;
		УменьшитьОстаток(СтрокаОстатковЗаГод.ОстатокДнейЗаГод,               ИспользованоДней);
...
Функция ПериодыСтажаПФР(Больничный, Кэш)
	...
	ПериодыПФР = ОбщегоНазначенияБЗК.ТаблицаПериодовИзТаблицыДат(ДатыНачислений, "ВидСтажаПФР2014, Состояние");
