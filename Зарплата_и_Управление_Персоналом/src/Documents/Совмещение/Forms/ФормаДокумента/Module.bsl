	...
	Если ЗначениеЗаполнено(Объект.ОтсутствующийСотрудник) Тогда
		...
		Если ДанныеСотрудника.Количество() > 0 Тогда
			ФизическиеЛицаЗарплатаКадры.Просклонять(
				ДанныеСотрудника[0].ФамилияИО, 2, ФамилияИООтсутствующегоСотрудника, ДанныеСотрудника[0].Пол)
		КонецЕсли;
...
Процедура ПрочитатьДанныеСовмещающегоСотрудника()
	...
	Если ЗначениеЗаполнено(Объект.СовмещающийСотрудник) Тогда
		...
		Если ДанныеСотрудника.Количество() > 0 Тогда
			ФизическиеЛицаЗарплатаКадры.Просклонять(
				ДанныеСотрудника[0].ФамилияИО, 2, ФамилияИОСовмещающегоСотрудника, ДанныеСотрудника[0].Пол)
		КонецЕсли;
