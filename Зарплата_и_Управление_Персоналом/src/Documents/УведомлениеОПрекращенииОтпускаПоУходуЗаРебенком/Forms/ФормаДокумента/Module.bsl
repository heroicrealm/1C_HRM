...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Организации"
		Или ИмяСобытия = "Запись_ЗаявлениеСотрудникаНаВыплатуПособия" Тогда
...
&НаКлиенте
Процедура ПредставлениеТелефонаСтрахователяПриИзменении(Элемент)
	Текст = Элемент.ТекстРедактирования;
	ПредставлениеТелефонаСтрахователяПриИзмененииНаСервере(Текст);
КонецПроцедуры

&НаКлиенте
Процедура ПредставлениеТелефонаСтрахователяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Оповещение = Новый ОписаниеОповещения("ПредставлениеТелефонаСтрахователяЗавершениеВыбора", ЭтотОбъект);
...
Функция ПараметрыФиксацииВторичныхДанных() Экспорт
	...
	ОписаниеЭлементовФормы.Вставить("ПредставлениеТелефонаСтрахователя", ОписаниеПутиКРеквизитамФормы);
...
Функция ФиксацияБыстрыйПоискРеквизитов()
	...
	Для Каждого КлючИЗначение Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		...
		КонецЕсли;
	КонецЦикла;
	БыстрыйПоискРеквизитов.Вставить("ПредставлениеТелефонаСтрахователя", "ТелефонСтрахователя");
...
&НаСервере
Процедура ПредставлениеТелефонаСтрахователяПриИзмененииНаСервере(Текст)
	...
	Иначе
		Значение = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(
			...
			ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонОрганизации"));
	КонецЕсли;
	ПослеВыбораАдресаИлиТелефонаНаСервере("ТелефонСтрахователя", Значение)
КонецПроцедуры

&НаКлиенте
Процедура ПредставлениеТелефонаСтрахователяЗавершениеВыбора(РезультатВыбора, ПараметрыОповещения) Экспорт
	Если ТипЗнч(РезультатВыбора) = Тип("Структура") Тогда
		ПослеВыбораАдресаИлиТелефонаНаСервере("ТелефонСтрахователя", РезультатВыбора.Значение);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьВидимостьДоступность()
	...
	КонтактнаяИнформацияБЗК.ОбновитьПолеВводаТелефона(
		ЭтотОбъект,
		"ПредставлениеТелефонаСтрахователя",
		Объект.ТелефонСтрахователя);
	...
	Если ЗначениеЗаполнено(Объект.НаименованиеСтрахователя) Тогда
		Массив = Новый Массив;
		Массив.Добавить(СтрШаблон(НСтр("ru = 'Страхователь: %1'"), Объект.НаименованиеСтрахователя));
		Если ЗначениеЗаполнено(ПредставлениеТелефонаСтрахователя) Тогда
			Массив.Добавить(ПредставлениеТелефонаСтрахователя);
		КонецЕсли;
		Если ЗначениеЗаполнено(Объект.АдресЭлектроннойПочтыСтрахователя) Тогда
			Массив.Добавить(Объект.АдресЭлектроннойПочтыСтрахователя);
		КонецЕсли;
		Элементы.ГруппаСведенияОСтрахователе.Заголовок = СтрСоединить(Массив, ", ");
	Иначе
		Элементы.ГруппаСведенияОСтрахователе.Заголовок = НСтр("ru = 'Сведения о страхователе не заполнены'");
	КонецЕсли;
...
Процедура ЗаполнитьПрекращаемыеЗаявленияНаСервере(ВыводитьОшибкиЗаполнения)
	...
	Отказ = Ложь;
	Если Не ВыводитьОшибкиЗаполнения Тогда
		Количество = СообщенияБЗК.КоличествоСообщенийПользователю();
	КонецЕсли;
	ОбъектМетаданных = Метаданные.Документы.УведомлениеОПрекращенииОтпускаПоУходуЗаРебенком;
	ПроверкиБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, Объект, "Сотрудник", , ОбъектМетаданных);
	ПроверкиБЗК.ПроверитьЗаполнениеРеквизитаОбъекта(Отказ, Объект, "ДатаПрекращенияОплаты", , ОбъектМетаданных);
	Если Отказ Тогда
		Если Не ВыводитьОшибкиЗаполнения Тогда
			СообщенияБЗК.СократитьЧислоСообщений(Количество);
		КонецЕсли;
		Возврат;
	КонецЕсли;
