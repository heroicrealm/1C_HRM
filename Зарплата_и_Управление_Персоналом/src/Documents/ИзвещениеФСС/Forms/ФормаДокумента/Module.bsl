...
Процедура СтрокиРеестровВыбор(ТаблицаФормы, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.СтрокиРеестровФизическоеЛицо Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы.Сотрудник) Тогда
			ОткрываемоеЗначение = СтрокаТаблицы.Сотрудник;
		Иначе
...
Процедура ОбновитьВидимостьДоступность()
	...
	МногострочныйРежим = Объект.СтрокиРеестров.Количество() > 1;
	ОднострочныйРежим  = Не МногострочныйРежим;
	
	Элементы.СотрудникГруппа.Видимость = (ЗначениеЗаполнено(Объект.Сотрудник) Или ОднострочныйРежим)
		И (ЭтоИзвещение
	...
	Если ЭтоИзвещение И ОднострочныйРежим Тогда
		...
		Элементы.ОпределятьАвтоматически.Видимость = 
			ЗначениеЗаполнено(Объект.ВходящееЗаявление)
			Или ЗначениеЗаполнено(Объект.ВходящийПервичныйДокумент);
