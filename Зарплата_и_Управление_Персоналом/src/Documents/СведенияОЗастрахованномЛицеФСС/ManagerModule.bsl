...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Макет = Метаданные.Документы.СведенияОЗастрахованномЛицеФСС.Макеты.ПФ_MXL_СведенияОЗастрахованномЛице_2021;
...
Процедура ПриФормированииПечатнойФормы(КонтекстПечати) Экспорт
	...
	Если КонтекстПечати.МетаданныеМакета = Макеты.ПФ_MXL_СведенияОЗастрахованномЛице_2021 Тогда
		ПриПечатиСведенийОЗастрахованномЛицеФСС_2021(КонтекстПечати);
	КонецЕсли;
...
Функция ТекстXML(Документ) Экспорт
	...
	Если ЗначениеЗаполнено(Документ.ДатаУвольнения) Тогда
		// Дата увольнения.
	...
	Иначе
		...
		Иначе
			...
			МенеджерXML.ДобавитьУзел(УзелXML, "urn:number",         УбратьПробелы(Документ.УдостоверениеЛичностиНомер));
			МенеджерXML.ДобавитьУзел(УзелXML, "urn:expirationDate", Документ.УдостоверениеЛичностиСрокДействия, Ложь, Ложь);
		КонецЕсли;
		...
		// Адрес регистрации.
		Если ЗначениеЗаполнено(Документ.АдресРегистрации)
...
Процедура ПриЗаполненииПараметровОтправки(МассивСсылок, ТаблицаОтправки) Экспорт
	...
	ИменаРеквизитов = "Ссылка, РегистрацияСведений, Номер, Дата, ДатаОтправки, ФизическоеЛицо, Организация, ХранилищеXML, ПометкаУдаления, Проведен";
	...
	Для Каждого КлючИЗначение Из Соответствие Цикл
		...
		Если Документ.ПометкаУдаления Тогда
			...
			Продолжить;
		КонецЕсли;
		Если ЗначениеЗаполнено(Документ.РегистрацияСведений) Тогда
...
Процедура ПриПечатиСведенийОЗастрахованномЛицеФСС_2021(КонтекстПечати)
...
Функция ВыборкаПоДокументам(МассивСсылок)
	...
	|	Сведения.ВнешнееСовместительство КАК ВнешнееСовместительство,
...
Функция ПоказыватьРазрешениеНаПроживание(Документ) Экспорт
	Возврат Не ЗначениеЗаполнено(Документ.КодСтраныГражданства)
		Или Не СЭДОФСС.ГражданствоРФ(Документ.КодСтраныГражданства);
КонецФункции

Функция ТребуетсяВыгрузитьРазрешениеНаПроживание(Документ) Экспорт
	Возврат ПоказыватьРазрешениеНаПроживание(Документ)
